...
the method are described in Ref. [@nardelli-prb99].
...
exist with a similar centre (e.g. *d*-orbital type Wannier functions
centred on a Cu atom). Next, consistent parities of Wannier function are
enforced. To distinguish between different types of Wannier function and
ascertain relative parities, a signature of each Wannier function is
...
up to third order in powers of sines. Here, the supercell
...
often output by plane-wave DFT codes. However, to calculate the 20
...
Tutorial of [how 2c2 calculations are performed](../../tutorials/tutorial_14.md)
