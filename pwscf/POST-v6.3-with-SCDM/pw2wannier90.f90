...
module wannier
   ...
   integer  :: vec_nproj(2)    ! vector with number of projection and WFs
...
SUBROUTINE read_nnkp
  ...
  INTEGER :: scdm_nproj !vv: needed to pass num_wann to SCDM
  ...
  IF (ionode) THEN   ! read from ionode only
     ...
     READ(iun_nnkp,*) vec_nproj(1), vec_nproj(2)
     n_proj = vec_nproj(1)
     IF (scdm_proj) THEN
        IF (n_proj == 0) THEN
           scdm_nproj = vec_nproj(2)
        ELSE
           WRITE(stdout,'(//," ****** begin WARNING ****** ",/)')
           WRITE(stdout,'(/," The wannier90 .win file contains a projection block",/)')
           WRITE(stdout,'(/," but scdm_proj = T in the pw2wannier90 input file.",/)')
           WRITE(stdout,'(/," Projections from the wannier90 input file will be discarded.",/)')
           WRITE(stdout,'(/," If this is NOT the intended behaviour please modify",/)')
           WRITE(stdout,'(/," the pw2wannier90 input file accordingly (scdm_proj = F).",/)')
           WRITE(stdout,'(/," ****** end WARNING ****** ",//)')
           scdm_nproj = vec_nproj(1)
           n_proj = 0
        ENDIF
     ENDIF
  ENDIF
...
SUBROUTINE compute_amn_with_scdm
   ...
   IF (wan_mode=='standalone') THEN
      ...
      IF (ionode) THEN
         WRITE (iun_amn,*) header
         WRITE (iun_amn,'(3i8,2f10.6)') numbands,  iknum, n_wannier, scdm_mu, scdm_sigma
      ENDIF
   ENDIF
