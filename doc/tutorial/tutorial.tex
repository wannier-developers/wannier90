...
\tent{The first plateau corresponds to atom-centered WFs of separate
  ...
  Sec.~IV.B of Ref.~\cite{wang-prb06}).  With hindsight, we can redo
  steps~4 and~5 more efficiently using trial orbitals with the same
  character as the final MLWFs,} {\tt
\begin{quote}
...
\tent{The second line instructs {\tt wannier90} to write in the output
  ...
  splittings for the $s$, $p$, and $d$ states separately. Compare
  their magnitudes with that observed in the DOS.}
...
uncomment the following instructions in {\tt Fe.win}, \smallskip {\tt
\begin{quote}
kslice = T
