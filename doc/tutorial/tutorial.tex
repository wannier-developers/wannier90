...
\tent{Physical quantities should come out essentially the same
  computed with any ``reasonable'' set of localized WFs, not
  necessarily maximally-localized.  Let us re-run steps~4 and~5 again
  starting from $s$, $p$, and $d$-type trial orbitals, but stopping
  the minimization of $\widetilde{\Omega}$ before the onset of
  hybridization,} {\tt
\begin{quote}
num\_iter = 50
\end{quote}
...
\tent{This tells {\tt wannier90} to write in the output file the
  ...
  Hamiltonian. The difference between corresponding values in {\tt
    iron\_up.wout} and in {\tt iron\_dn.wout} can be interpreted as
  the exchange splittings for the individual $s$, $p$, and $d$-type
  WFs. Compare their magnitudes with the splittings displayed by the
...
contours''), can be visualized with the script {\tt
  Fe\_energy\_cntr.gnu} generated at runtime, {\tt
\begin{quote}
myshell> gnuplot

gnuplot> load `Fe\_energy\_cntr.gnu'
\end{quote} }
...
\item Redraw the Fermi-surface contours on the (010) plane starting
  ...
  in Example~8). Compare with Fig.~\ref{fig:fe-fermicontours} to see
  how the spin-orbit interaction changes the connectivity of the Fermi
  surface.
...
Note that the sequence of steps below is the same of Example~17.  If
you have already run that example, you can reuse the output files from
...
The  Berry curvature along $z$ of a Bloch state $\psi_{n{\bf k}}=e^{i{\bf k}\cdot {\bf r}}u_{n{\bf k}}$ is defined as
$$
...
Fermi-surface contours on the (010) plane in $k$-space.  To visualize
$-\Omega^z({\bf k})$ across the plane, uncomment the following
instructions in {\tt Fe.win}, \smallskip {\tt
\begin{quote}
kslice = T
...
and re-run {\tt postw90}.  For the visualization we use the {\tt
  python} script {\tt Fe\_curv\_heatmap.py} generated at runtime, 
{\tt
\begin{quote}
myshell> python Fe\_curv\_heatmap.py
\end{quote} }
\smallskip

which produces a heatmap plot of the Berry curvature, similar to
...
$\sigma_{xy}=-\sigma_{yx}$.  To evaluate $\sigma_{xy}$ by
integrating $-\Omega^z({\bf k})$ over a $25\times 25\times 25$
$k$-point mesh, uncomment the following lines in {\tt Fe.win},
{\tt
\begin{quote}
berry = T
...
\item Study the convergence of the AHC with respect to the density of
  ...
  \item Progressively increase the value of the
    parameter {\tt berry\_interp\_mesh}.

  \item In addition to increasing the mesh density uniformly across
    the BZ, it helps to adaptively refine it in regions where the
    Berry curvature is large and rapidly varying (such regions can be
    seen in Fig.~\ref{fig:fe-curv-slice}).  Add to {\tt Fe.win} the
    lines \smallskip {\tt
      \begin{quote}
    ...
    in order to add a $5\times 5\times 5$ fine grid around those
    points where $\vert\Omega^z({\bf k})\vert$ exceeds
    27.98~\AA$^2\simeq 100$~bohr$^2$. The percentage of mesh points
    triggering adaptive refinement is given in {\tt Fe.wpout}.

  \item Compare the converged value for the AHC of bcc Fe with the
    values reported in Refs.~\cite{wang-prb06} and~\cite{yao-prl04}.
...
\item {\bf [\underline{POSSIBLE ADDITION:}]} Explain how to modify the
  ...
  along lines and planes in $k$-space. Basically, need to add {\tt
    write\_uHu = .true.} to {\tt Fe.pw2wan} in order to evaluate the
  matrix elements $\langle u_{n{\bf k}+{\bf b}_1}\vert H_{\bf k}\vert
  u_{m{\bf k}+{\bf b}_2}\rangle$, and re-run step~4. Then replace {\tt
    curv/curv\_heatmap/ahc} with {\tt morb} everywhere in the {\tt
    kpath}, {\tt kslice}, and {\tt berry} input variables, and re-run
  step~6.
