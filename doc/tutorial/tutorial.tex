...
\begin{itemize}
\item {\footnotesize[solved] IS: With the growing number of examples, it may be helpful
  to add an index, like we have for the User Guide (and make examples
  titles more descriptive: I started doing that...)

GP: I added the
  table of contents at the beginning. In the future, use the new
  command \verb#\sectiontitle{blablabla}# instead of
\verb#\section*{blablabla}#}
\item IS: For same reason, we should think of using bibtex for the
  growing number references (to automatically order them). GP: I agree.
\item {\footnotesize [done] IS: Replace MLWF $\rightarrow$ MLWFs? (like in RMP
  paper) GP: I changed it where appropriate.}
\item {\footnotesize [solved]IS: Underscores are not searcheable in the pdf. Is there a way
  of changing that (here and also in User Guide)? It's a pity, since
  most keywords have underscores in them. 

GP: I added the \verb#\usepackage[T1]{fontenc}# package that solved this
issue to both documents.}

\item {\footnotesize
I added the hyperref package to have links from the TOC to the
chapters. I also changed all links using the \verb#\url{}# command 
(and including always http://) so that one can simply click on the
link in the pdf to open the link.
}
...
\item{\tt python} with the {\tt numpy} and {\tt matplotlib} modules
  (used e.g. in example 18 on Berry curvature)\\
\tent{add here links}
\end{itemize}

\sectiontitle{Parallel execution}
\label{sec:parallel}
{\tentbis GP: I added here this subsection, feel free to
  edit/modify/move. If you move it, please update references to
  sec:parallel later in the examples.

Presently, the \wannier\ is instead a serial-only executable, so
it \emph{cannot} be run in parallel using MPI libraries.

On the contrary, the {\tt postw90} executable can be run in parallel
to speed up the calculations, using the MPI libraries.

To enable the parallel version, you must specify some flags in the
{\tt make.sys} file of {\tt wannier90} and {\tt postw90}; for further information, please refer to the {\tt
  README.install} file in the top directory of the {\tt wannier90}
distribution.
...
such as {\sc 
  castep} (\url{http://www.castep.org}), {\sc abinit} (\url{http://www.abinit.org}), and {\sc fleur} (\url{http://www.flapw.de}), are in progress \tent{{\bf [CHECK: still in progress?]}}.
...
\sectiontitle{1: Gallium Arsenide \tent{-- MLWFs for the valence bands}}
...
wannier\_\-plot = true
\end{quote} }
and re-running \wannier. To visualise the MLWFs we must
...
\sectiontitle{2: Lead \tent{-- Wannier-interpolated Fermi surface}}
...
\sectiontitle{3: Silicon \tent{-- Disentangled MLWFs}}

\begin{itemize}
\item{Outline: \it{Obtain \tent{disentangled} MLWFs for the valence and low-lying
...
\sectiontitle{4: Copper \tent{-- Fermi surface, orbital character of
    energy bands}}
...
\tent{Ivo: Should we explain somewhere in the tutorial
...
{\tentbis GP: I try to add the following, to decide if this is the
  correct place. Feel free to modify/adapt/correct. I put here only
  the discussion for the parallel execution of pw2wannier90, and
  instead the part for wannier+postw90 in the preliminaries.

 Note that, as {\tt pw.x}, also the {\tt pw2wannier90.x} executable can be run in
parallel (and for large calculations, this is extremely useful to
significantly reduce the computation time).
This requires that the code is compiled in its parallel version, using
the MPI libraries. Refer to the \QE\ package for the documentation on
how to compile the code in parallel.
Note that, unless you specify {\tt wf\_collect=.true.} in your {\tt pw.x}
input file, you must run {\tt pw2wannier90} with the same number of
processors as {\tt pw.x}.

Moreover we remind here that as discussed in the ``Parallel execution'' section at page
\pageref{sec:parallel}, while the \wannier\ executable is serial and cannot be run
in parallel, the {\tt postw90.x} can be run in parallel and in this case any number of processors
can be used, independently of what has been used for {\tt pw.x} and
{\tt pw2wannier90.x}.
}

\cleardoublepage

\sectiontitle{5: Diamond \tent{-- MLWFs for the valence bands}}
\begin{itemize}
...
\sectiontitle{6: Copper \tent{-- Fermi surface}}
...
\sectiontitle{7: Silane  (SiH$_4$) \tent{-- Molecular MLWFs using 
...
\sectiontitle{8: Iron \tent{-- Spin-polarized WFs, total and projected DOS}}
\begin{itemize}
\item{Outline: \it{\sout{Obtain MLWFs to describe the states around the
...
\end{itemize}
\item{Note that we obtain the MLWFs for spin up and spin down in
...
\sectiontitle{11: Silicon \tent{-- Valence and conduction states, scissors correction}}
...
\begin{itemize}
\item Compare the Wannier interpolated bandstructure with the full
  \pwscf\ bandstructure. Compute MLWFs using a finer
...
\sectiontitle{13: (5,5) Carbon Nanotube \tent{-- Transport properties}}
...
\sectiontitle{14: Linear Sodium Chain \tent{-- Transport properties}}
...
\sectiontitle{15: (5,0) Carbon Nanotube \tent{-- Transport properties}}
...
fact that not all bands cross the Fermi energy.) Alternative, the
python script {\tt Fe-energy\_cntr.py} can be used,
%
{\tt
\begin{quote}
...
\sectiontitle{18: Iron -- Berry curvature and anomalous Hall 
conductivity}
  ...
  \item[{\bf --}] In addition, it helps to adaptively refine the mesh
    ...
    to {\tt Fe.win} the lines \smallskip {\tt
      \begin{quote}
        ...
        berry\_adpt\_kmesh\_thresh = 27.98
      \end{quote} }
...
{\tt postw90} as the BZ integral of a quantity ${\bf M}_{\rm orb}({\bf
  k})$ similar to the Berry curvature (see Ch.~11 in the
{\tt wannier90} User Guide)
%
{\tt
\begin{quote}
berry = T
...
\sectiontitle{20: Iron -- Optical conductivity and joint density-of-states}
...
{\tt spin\_decomp} in ``Further ideas.''}
...
\item More generally, the Kramers-Kronig relation allows to compute
  the AHC at finite frequencies, starting from the MCD spectrum:
$$
\sigma^{\rm AH}_{xy}(\omega)=\frac{2}{\pi}\int_0^\infty\,
\frac{{\rm Im}\,\sigma^{\rm A}_{xy}(\omega')}{\omega'^2-\omega^2}\,d\omega'
$$
%
Write a script/program that reads the MCD datafile generated by {\tt
  postw90}, and carries out the (truncated) Kramers-Kronig integral
over frequencies to calculate $\sigma^{\rm AH}_{xy}(\omega)$. Plot the
result versus $\omega$, and compare with Fig.~5 (lower panel) of
Ref.~\cite{yao-prl04}.
...
\bibitem{UserGuide} A.~A.~Mostofi and J.~R.~Yates, \wannier: User
  Guide, available at \url{http://www.wannier.org/user\_guide.html}.
...
\bibitem{BaTiO3} N.~Marzari and D.~Vanderbilt, Maximally-Localized
  ...
  \url{http://arxiv.org/abs/cond-mat/9802210}.
