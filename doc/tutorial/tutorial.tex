...
\begin{itemize}
	\item{Outline: \it{Calculate the spin Hall
			conductivity (SHC) of fcc Pt with spin-orbit coupling. In preparation for this example
			it may be useful to read Ref.~\cite{qiao-prb2018} for a detailed 
...
The band-projected Berry curvature-like term $\Omega_{n,\alpha\beta}^{\text{spin} \gamma}({\bm k})$ 
is defined in Eq.~(12.22) of the User Guide.  The following
lines in {\tt Pt.win} are used to calculate the energy bands and the
band-projected Berry curvature-like term (in \AA$^2$) along high-symmetry lines in $k$-space, i.e. the {\tt kpath} plot.
{\tt
	\begin{quote}
...
However, you may adjust the {\tt kubo\_smr\_fixed\_en\_width} 
as you like to draw a visually appealing plot. 
A {\tt kpath} plot of 0.05 eV smearing is shown in the 
Solution Booklet.

Similarly, we can get a heatmap plot of the k-resolved Berry curvature-like term 
$\Omega_{\alpha\beta}^{\text{spin}\gamma}({\bm k})$, i.e. the {\tt kslice} plot. To move forward, 
...
\end{quote} }
%
Note the {\tt kslice\_b2} is actually $(\frac{\sqrt{2}}{4},   \frac{3\sqrt{2}}{4},0.0)$ which makes the slice a square in the Brillouin zone, 
easing the plotting in the generated {\tt python} script.  Re-run {\tt postw90}, and issue%
{\tt
	\begin{quote}
...
\medskip
%
Compare with Fig.~3 in Ref.~\cite{qiao-prb2018}. 

\subsection*{Notes}
%
\begin{itemize}
\item The {\tt kpath} and {\tt kslice} plots are incompatible 
with adaptive smearing. So a fixed smearing is needed to avoid 
...
\end{quote} }
with {\tt 1, 2, 3} stands for {\tt x, y, z} respectively.
\end{itemize}
...
{\tt
	\begin{quote}
		...
		\#kubo\_smr\_fixed\_en\_width = 1
\end{quote} }
The set the Fermi energy as a value
{\tt
	\begin{quote}
		fermi\_energy = [insert your value here]
\end{quote} } 
...
SHC at this specific value; If a list of Fermi energies are set, 
...
sampling, and a $25\times 25\times 25$ does not yield a well-converged value.
...
\begin{itemize}
	...
	\item[{\bf --}] To accelerate the convergence, adaptively refine the
	mesh around spikes in the Berry curvature-like term, by adding to {\tt
...
This adds a $5\times 5\times 5$ fine mesh around those points
where $\vert{\Omega_{\alpha\beta}^{\text{spin}\gamma}}({\bf k})\vert$ exceeds 100~{\tt berry\_curv\_unit}. The
percentage of points triggering adaptive refinement is reported in
{\tt Pt.wpout}.

Compare the converged SHC value with those obtained in
Refs.~\cite{qiao-prb2018} and~\cite{yao-prl04}.

Finally, we provide a list of input parameters that 
...
\end{quote} }
Their meanings and usages can be found in the User Guide.

\sectiontitle{30: Gallium Arsenide -- Frequency-dependent spin Hall conductivity}

\begin{itemize}
	\item{Outline: \it{Calculate the alternating current (ac) spin Hall
			conductivity of GaAs with spin-orbit coupling. In preparation for this example
			it may be useful to read Ref.~\cite{qiao-prb2018} for a detailed 
			description of the theory and Ch.~12.5 of the User Guide.}}
...
The dc SHC calculated
earlier corresponds to $\sigma_{\alpha\beta}^{\text{spin}\gamma}$ in the limit $\omega\rightarrow
...
%
and re-run {\tt postw90}. Reasonably converged spectra can be
obtained with a $250\times 250\times 250$ $k$-point mesh. Let us first
plot the ac SHC in $\frac{\hbar}{e}$S/cm, as in the Fig.~4 in
Ref.~\cite{qiao-prb2018}, {\tt
	\begin{quote}
		myshell> gnuplot
...
{\tt
	\begin{quote}
		...
		gnuplot> plot `GaAs-shc-freqscan.dat' u 1:(\$1)*(\$3)*0.0137 w l, 
\end{quote} }
...
\begin{itemize}
	...
	following keyword\smallskip {\tt
		\begin{quote}
			scissors\_shift = 1.1171896290719894
