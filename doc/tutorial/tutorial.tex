...
\section*{8: Iron \tent{-- Spin-polarized WFs, total and orbital-projected DOS}}
\begin{itemize}
...
\item \tent{In order to obtain the DOS projected onto the $p$-type WFs
    (in the same spirit as done in Example~4 for the energy bands),
    add to the {\tt .win} files the lines} {\tt
\begin{quote}
dos\_project = 2,3,4
\end{quote}
}
...
\item \tent{Add to the {\tt .win} files the instruction}
{\tt
\begin{quote}
write\_hr\_diag = T
\end{quote}
...
\tent{This tells {\tt wannier90} to write in the output
...
contours''), can be visualized with the script {\tt
  Fe\_energy\_cntr.gnu} generated during runtime, {\tt
\begin{quote}
myshell> gnuplot
...
\item Study the convergence of the AHC with respect to the density of
  ...
  \item In addition to increasing the mesh density uniformly across
    ...
    Berry curvature is large and rapidly varying (see
