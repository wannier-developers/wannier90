...
\begin{itemize}
\item Try to disentangle the Wannier functions without using the mesh. You will see that this is extremely hard due to the bands touching only near the $A$ point (for unstrained LaVO$_3$, instead, there is a gap so disentanglement is not needed).
\item Try to run similar simulations for SrMnO$_3$. The input files can be found
in the SrMnO$_3$ subfolder. There are three different {\tt .pw2wan} and {\tt .win} files for projections on different states. Accordingly, different spheres are taken into account.
\end{itemize}
