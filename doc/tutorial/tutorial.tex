...
$-\bm{\Omega}({\bf k})$ along lines in $k$-space, {\tt
\begin{quote}
...
in $k$-space.  To visualize both the Fermi contours and
...
which produces a heatmap plot of $-\Omega^z({\bf k})$, similar to
...
\end{quote} } 
(the last line can be simplified to {\tt berry\_interp\_mesh = 25}) 
and re-run {\tt postw90}.  The AHC in S/cm is written in the output
file {\tt Fe.wpout} in (pseudo)vector form. The $x$, $y$, and $z$
components correspond to $\sigma^{\rm AH}_{yz}$, $\sigma^{\rm
  AH}_{zx}$, and $\sigma^{\rm AH}_{xy}$ respectively. For bcc Fe with
the magnetization along [001] the only nonzero component is
$\sigma_{xy}$. 
...
\item The Berry curvature in bcc Fe results from the magnetic order
  which, together with spin-orbit, breaks time-reversal in the orbital
  wavefunctions. Another way of inducing a Berry curvature is to break
  spatial inversion (no spin-orbit needed).

  Plot the Berry curvature of the valence bands of GaAs over selected
  planes in the BZ. Note that while locally nonzero, its BZ integral
  vanishes because $\bm{\Omega}(-{\bf k})=-\bm{\Omega}({\bf k})$.
  
\tent{{\bf [Need to provide input files for GaAs?]}}
