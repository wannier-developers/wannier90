...
\tent{{\bf [Need to provide input files for GaAs and test them]}}
...
After running {\tt postw90}, issue from the terminal
{\tt
\begin{quote}
myshell> gnuplot
...
To plot the DOS using {\tt gnuplot}, issue
%
{\tt
\begin{quote}
myshell> gnuplot
...
\end{quote} }
%
(As indicated in {\tt Fe.wpout}, the symmetric conductivity is plotted
in units of $10^{15}$~sec$^{-1}$.) The difference in shape between the
JDOS and the optical conductivity can be attributed to the variability
in the dipole matrix elements describing the optical transitions.
Note also that the calculate spectrum is ``sharp,'' while experimental
spectra are broadened by lifetime effects which are not taken into
...
magnetization direction $\hat{\bf z}$. To plot the
...
$10^{29}$~sec$^{-2}$. Compare the spectrum with the one displayed
in Ref.~\cite{yao-prl04}. To improve the agreement, you need to increase
...
and $\omega_{\rm min}$ is scanned in the range $[0,\omega_{\rm
  max}]$. To plot the result (in S/cm) versus $\omega_{\rm min}$, issue
%
{\tt
\begin{quote} myshell> gnuplot
...
\end{quote} }
%
Compare the value for $\omega_{\rm min}=0$ with the value of the AHC
calculated in Example~18 by directly integrating the Berry curvature.
