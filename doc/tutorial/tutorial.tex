...
\sectiontitle{34: InSb -- Nernst effect}
\begin{itemize}
\item{Outline: \it{Achieve MLWFs for the valence and low-lying
    conduction bands of InSb. Compute the isothermal Nernst, isothermal Hall, and Ettingshausen coefficients  within the constant relaxation time approximation the \nw\ module.}} 
\end{itemize}
\begin{itemize}
  \item{Directory: {\tt examples/example34}}
  \item{Input Files}
    \begin{itemize}
      \item{ {\tt InSb.scf}  {\it The \pwscf\ input file for ground state
	  calculation}}
      \item{ {\tt InSb.nscf}  {\it The \pwscf\ input file to achieve Bloch
	  states for the conduction states}} 
      \item{ {\tt InSb.pw2wan}  {\it Input file for {\tt pw2wannier90}}}
      \item{ {\tt InSb.win}  {\it The {\tt wannier90} input file}}
    \end{itemize}
\end{itemize}


\begin{enumerate}
\item Run \pwscf\ to achieve the ground bands of InSb\\  
{\tt pw.x < InSb.scf > scf.out}

\item Run \pwscf\ to achieve the Bloch states on a uniform k-point
  grid.\\ 
{\tt pw.x < InSb.nscf > nscf.out}

\item Run \wannier\ to generate a list of the required overlaps (written
  into the {\tt InSb.nnkp} file).\\
{\tt wannier90.x -pp InSb}

\item Run {\tt pw2wannier90} to obtain the overlap between Bloch
  states and the projections for the starting guess (written in the
  {\tt InSb.mmn} and {\tt  InSb.amn} files).\\
{\tt pw2wannier90.x < InSb.pw2wan > pw2wan.out}
...
{\tt postw90.x InSb} (serial execution) \\
{\tt mpirun -np 8 postw90.x InSb} (parallel execution) 
\end{enumerate}

The output file {\tt InSb.wpout} shows the details of the calculation. Ensure that there are no errors and no unwanted flags are passed to \nw . The first order ({\tt InSb\_tdf1.dat}) and second order ({\tt InSb\_tdf2.dat}) transport distribution functions are saved in two files. You will need to download the post processing "NERTAW.py" code form the GitHub page of the author (Emad-Rezaei) to calculate the isothermal Hall conductivity ({\tt InSb\_Hall_T.dat}), isothermal Nernst coefficient ({\tt InSb\_Nernst_T.dat}), and Ettingshausen coefficient ({\tt InSb\_Etn.dat}). The detailed information is available in the documentation of the "NERTAW" code. 
