...
\caption{Bandstructure of strained LaVO$_3$. Black: ab-initio bands; red circles: Wannier-interpolated band structure. In order to disentangle the bands, only a small sphere around the A point has been used.} 
\label{fig:lavo3}
\end{center}
\end{figure}
...
\item Try to run similar simulations for SrMnO$_3$. The input files can be found in the SrMnO$_3$ subfolder. There are three different {\tt .pw2wan} and {\tt .win} files for projections on different states. Accordingly, different spheres are taken into account. Note that if one in interested in disentangling in points with degeneracy larger than 1 (e.g. the X point), then one has to include all symmetry-equivalent X points as centers of the spheres. This was not necessary for LaVO$_3$ because the A point is not degenerate, but it is important for the X point of SrMnO$_3$.
\end{itemize}
