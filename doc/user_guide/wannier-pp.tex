...
There is a seamless interface between \wannier\ and \pwscf, a
plane-wave DFT code that comes as part of the {\tt quantum-espresso}
package (see {\tt www.quantum-espresso.org} or {\tt
  www.pwscf.org}). At the time of writing, interfaces to other
electronic structure codes are in progress, e.g., {\sc castep} ({\tt
  www.castep.org}), {\sc abinit} ({\tt www.abinit.org}) and {\sc
  fleur} ({\tt www.fleur.de}). But, for the time being, you will need
...
\item   \verb#write_unk# -- Set to \verb#.true.# to write the periodic part
   of the Bloch functions for plotting or for automated lcr calculations in \wannier. Default is
   \verb#.false.#
