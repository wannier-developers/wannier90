...
This section explains how to calculate Wannier functions using the wave functions in the irreducible BZ (IBZ).
Currently, this feature is implemented only in the interface code with {\sc Quantum ESPRESSO}.
...
\item Run {\tt pw2wannier90} with \verb#irr_bz = .true.# in the input file.
  ...
  \verb#seedname.isym#.
\item Run a python code, \verb#write_full_data.py#, in {\tt symWannier} package to generate
  \verb#seedname.mmn#, \verb#seedname.amn#, and \verb#seedname.eig#.
  See also https://github.com/wannier-utils-dev/symWannier.
\item Run \verb#wannier90# with \verb#postproc_setup#.
\end{enumerate}
...
\begin{verbatim}
  ...
  R(1,3)   R(2,3)   R(3,3)
   t(1)     t(2)     t(3)
   T
  u(1,1).real  u(1,1).imag  (spinor case)
  u(2,1).real  u(2,1).imag  (spinor case)
  u(1,2).real  u(1,2).imag  (spinor case)
  u(2,2).real  u(2,2).imag  (spinor case)
   invs
\end{verbatim}
