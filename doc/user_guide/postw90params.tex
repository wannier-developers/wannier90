...
\item \texttt{berry}: Calculation of properties related to the BZ
  ...
  (see Chap.~\ref{ch:berry} and examples 18, 19, 25 and 29 of the
  tutorial).
...
\item[{\bf --}] \verb#shc# Spin Hall conductivity, in $\frac{\hbar}{e}$S/cm.  Output files:

	\begin{itemize}
	
	\item[$\cdot$] {\tt seedname-shc-fermiscan.dat} (data file). The
	first column is the number of the entry in the list, the second column contains the Fermi level $\varepsilon_F$ in eV, and the
