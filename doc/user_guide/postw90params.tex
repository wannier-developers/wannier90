...
\textcolor{red}{{\bf [Need to code this!]}}
...
\item[{\bf --}] \verb#morb_heatmap# Integrand of the $k$-space orbital
  ...
  \item[$\cdot$] {\tt seedname-slice\_morb.dat}: Berry curvature 
    on the grid
    
  \item[$\cdot$] {\tt seedname-morb\_\{x,y,z\}.py}: {\tt python} scripts
...
$m\times n$ grid.  If only one integer $m$ is given, an $m\times
m\times m$ grid is used.
...
Energy level in eV for plotting isoenergy contour lines when {\tt
