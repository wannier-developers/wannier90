...
\item \verb#integer,dimension(3,num_kpts,num_nnmax),#
      ...
      $M_{mn}^{(\mathbf{k,b})}$.
\item  \verb#real(kind=dp), dimension(3,num_bands), intent(out) :: proj_site# \\
Projection function centre
in crystallographic co-ordinates relative to the direct lattice
vectors.
\item \verb#integer, dimension(num_bands), intent(out) :: prog_l#\\
...
\item \verb#real(kind=dp), dimension(num_bands), intent(out) :: proj_zona#\\
The value of $\frac{Z}{a}$ associated
with the radial part of the atomic orbital. Units are in reciprocal
Angstrom.
\item \verb#real(kind=dp),dimension(num_bands), intent(out) :: proj_box#\\ 
The linear dimension of the real-space
...
currently used.
\item \verb#integer, dimension(num_bands), intent(out) :: exclude_bands#\\ 
