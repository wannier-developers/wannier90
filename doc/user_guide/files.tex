...
When performing selective localization only few things change in the output file
and in general the same principles shown in the above section will apply. It is strongly
reccomanded, that the Reader has acquired some familiarity with   
the original work of Marianetti \textit{et al.} \cite{Marianetti}. 
...
$$\Omega^{'} = \sum_{n=1}^{J'<J} \left[\langle r^2 \rangle_n - \overline{\mathbf{r}}_n\right]$$
and 
...
The total number of Wannier function is $J$, whereas $J'$ is the number functions to be selectively localized.
...
is given in the {\tt MAIN} section
\begin{verbatim}
 *---------------------------------- MAIN ------------------------------------*
...
The next part of the output files provides information of the minimisation of the 
new spread functional.
\begin{verbatim}
 *------------------------------- WANNIERISE ---------------------------------*
 +--------------------------------------------------------------------+<-- CONV
 | Iter  Delta Spread     RMS Gradient      Spread (Ang^2)      Time  |<-- CONV
...
When comparing the output from a selective localization calculation with a standard 
wannierisation (see previous section), there only differences are in the definition of
...
functional is $\Omega^{'}$.    
The part on the final state has one more information: the value of the difference
between the global spread functional and the new spread functional given by 
\texttt{Omega Rest}
$$\Omega_{R} = \sum_{n=1}^{J-J'} \left[\langle r^2 \rangle_n - \overline{\mathbf{r}}_n \right]$$

If adding constrains to the centres of the selectively localized Wannier functions,
 ...
 *----------------------------------------------------------------------------*
\end{verbatim}
As before one can check that the selective localization with constrains is on by
...
The output file for the minimisation section is modified as follow: 
both {\tt O\_IOD} and {\tt O\_TOT} now take into account
the factors coming from the new term in the functional due to the constraint,
which is implemented using the following penalty functional
$$\lambda_c \sum_{n=1}^{J'} \left(\overline{\mathbf{r}}_n - \mathbf{r}_{0n} \right)^2,$$
where $\mathbf{r}_{0n}$ is the desired centre for the $n$-th Wannier function, see \cite{Marianetti}.
...
of the constraint {\tt Penalty func}, which should be zero if the final centres
of the Wannier functions lie on top of the desired centres
\begin{verbatim}
 Final State
...
\section{{\tt seedname\_band.labelinfo.dat}}
OUTPUT. Written if {\tt bands\_plot=.TRUE.}; Information on the high-symmetry
point locations and labels used in the band plot of interpolated band structure.
It contains one line per high-symmetry point. If the last point of a path segment
is identical to the first of the next segment (both label and coordinates), the 
point is not repeated twice. Each line consists of six components: the label of
the k-point as specified in input (truncated to 20 characters, and converted
to capital letters); the index in the {{\tt seedname\_band.kpt}}
(1 is the first point) corresponding to this high-symmetry point, 
the $x$ value that is used in the {{\tt seedname\_band.dat}} for this point,
and the three coordinates of the k-point (same units as the 
{{\tt seedname\_band.kpt}} file).
