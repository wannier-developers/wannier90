...
{\sc jprime} & I & The number of objective Wannier Functions \\
{\sc lambdac} & R & The lagrange multiplier in the constrained \Omega \\
\hline
\end{tabular}
...
The constrained centres block defines the centers and the Lagrange multipliers for the WFs whose centers are meant to be constrained. 
...
The above block shows all possible ways of setting the constraints. In particular :
\begin{itemize}
\item The first line sets the constrain for the centre of Wannier function number 2 (as defined in the projection block) to (0,0,0) in fractional co-ordinates with $\lambda_2$ = 1 (default value if fifth column is missing).
\item The second line sets the constrain for the centre of Wannier function number 5 (as defined in the projection block) to (1,0,0.5) in fractional co-ordinates with $\lambda_5$ = 10
\item The third line sets the constrain for the centre of Wannier function number 7 (as defined in the projection block) to it's initial projection centre with $\lambda_7$ = 1
\item The centres of all the other Wannier functions are set to their initial projection centres with $\lambda_n$ equal to {\tt lambdac} (which defaults to 0).
\end{itemize}
...
\subsection[lambdac]{\tt real(kind=dp) :: lambdac}
Global value of the Lagrange multiplier $\lambda$ in the spread functional $\Omega_c$ from the constrainted centres algorithm of Marianetti \textit{et al} \cite{Marianetti}. It must be a positive real number. A value of $0.0$ means that no constraints are applied to the original spread functional.
