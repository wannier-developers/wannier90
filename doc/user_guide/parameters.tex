...
{\sc sel\_loc\_num} & I & The number of objective WFs for selective localization \\
{\sc sel\_loc\_constrain} & L & Whether to constrain the centres of the objective WFs \\
{\sc sel\_loc\_constrain\_lambda} & R & Value of the Lagrange multiplier for constraining the objective WFs \\
{\sc centre\_constraints} & P & The centres to which the objective WFs are to be constrained \\
\hline
\end{tabular}
...
\subsection[sel\_loc\_num]{\tt integer :: sel\_loc\_num}
The number of objective Wannier functions for selective localisation in the selectively localised Wannier function (SLWF) method of Ref.~\cite{Marianetti}. These functions are obtained by minimising the spread functional only with respect to the degrees of freedom of a subset of {\tt sel\_loc\_num} $\le$ {\tt num\_wann} functions. At convergence, the objective WFs will have a minimum cumulative spread, whereas the remaining {\tt num\_wann} $-$ {\tt sel\_loc\_num} functions are left unoptimised. The initial guesses for the objective WFs are given by the first {\tt sel\_loc\_num} orbitals in the {\tt projections} block. 

The default is {\tt num\_wann}.

\subsection[sel\_loc\_constrain]{\tt logical :: sel\_loc\_constrain}
If {\tt sel\_loc\_constrain=true}, then the centres of the objective Wannier functions are constrained to either the centres of the first {\tt sel\_loc\_num} orbitals in the {\tt projections} block or to new positions specified in the {\tt centre\_constraints} block (see Sec.~\ref{sec:centre_constraints}). In this case, a modified spread functional, $\Omega_c$, with the addition of a constraint term, as described in Ref.~\cite{Marianetti}.

The default is {\tt false}

\subsection[sel\_loc\_constrain\_lambda]{\tt real(kind=dp) :: sel\_loc\_constrain\_lambda}
The value of the Lagrange multiplier $\lambda$ for the constraint term in term added to modify the spread functional: $ \lambda \sum_{n=1}^{J'} \left(\overline{\mathbf{r}}_n - \mathbf{r}_{0n}\right)$, where $J'$ is {\tt sel\_loc\_num} and $\mathbf{r}_{0i}$ is the desired centre for the $i^{\text{th}}$ objective WF. 
...
If {\tt sel\_loc\_constrain=true}, then by default the centres to which the {\tt sel\_loc\_num} objective Wannier function centres are constrained are given by the first {\tt sel\_loc\_num} rows of the {\tt projections} block. 

Optionally, the {\tt centre\_constraints} block may be used to define alternative target centres for some or all of the {\tt sel\_loc\_num} objective Wannier functions.
