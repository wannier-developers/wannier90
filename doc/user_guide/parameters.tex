...
The value of the Lagrange multiplier $\lambda$ for the constraint term in term added to modify the spread functional: $ \lambda \sum_{n=1}^{J'} \left(\overline{\mathbf{r}}_n - \mathbf{r}_{0n}\right)$, where $J'$ is {\tt slwf\_num} and $\mathbf{r}_{0i}$ is the desired centre for the $i^{\text{th}}$ objective WF. 
