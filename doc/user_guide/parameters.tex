...
{\sc constrain\_centres} & L & To constrain the centres of the OWFs \\
{\sc constrain\_centres\_tol} & R & The convergence tolerance for the constrained $\Omega$ \\
\hline
\end{tabular}
...
\subsection[constrain\_centres]{\tt logical :: constrain\_centres}
If \tt{true} the centres of the objective Wannier Functions are constrained to either the centres of the projection functions or to new positions specified in the \tt{centre\_constrains} block. A modified spread functional, $\Omega_c$, with the addition of a penalty function, is minimized in this case, see Marianetti \textit{et al} \cite{Marianetti}. 

The default is {\tt false}

\subsection[constrain\_centres\_tol]{\tt real(kind=dp) :: constrain\_centres\_tol}
In the constrained centres approach the minimisation is said to be converged when either the change in $\Omega_c$ over {\tt
  conv\_window} successive iterations is less than {\tt
  conv\_tol} (these keywords must be defined in the input file) or the change in the target WF centres is less than {\tt constrain\_centres\_tol}. If these two criterion are not satisfied the algorithm ends after {\tt num\_iter} iterations.
