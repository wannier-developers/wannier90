...
Fermi energy in the specified range\\
  ...
  surface plot \\
  {\sc hr\_plot} & L & This parameter is not used anymore. Use {\sc write\_hr} instead. \\
  {\sc write\_hr} & L & Write the Hamiltonian in the WF basis \\
  {\sc write\_rmn } & L & Write the position operator in the WF basis \\
{\sc write\_tb }  & L & Write lattice vectors, 
Hamiltonian, and position operator in WF basis\\
  ...
  {\sc write\_u\_matrices } & L & Write $\mathbf{U}^{(\mathbf{k})}$ and $\mathbf{U}^{\mathrm{dis}(\mathbf{k})}$ matrices to files \\
