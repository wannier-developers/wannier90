...
{\sc sel\_loc\_num} & I & The number of Objective Wannier Functions (OWFs) for selective localization \\
{\sc sel\_loc\_constrain} & L & Whether to constrain the centres of the OWFs \\
{\sc sel\_loc\_constrain\_lambda} & R & Value of the Lagrange multiplier $\lambda$ 
associated to the constrain on the centres of the OWFs \\  
\hline
\end{tabular}
...
The number of Wannier functions for selective localisation as defined in \cite{Marianetti}. These functions are obtained by minimising the spread functional only with respect to the degrees of freedom of a subset of {\tt sel\_loc\_num} $<$ {\tt num\_wann} functions. Here we follow the original nomenclatur are named objective Wannier Functions (OWFs). At convergence, the OWFs will have a minimum cumulative spread, whereas the remaining {\tt num\_wann} - {\tt sel\_loc\_num} functions are left unoptimised. The initial guesses for the OWFs are given by the first {\tt sel\_loc\_num} orbitals in the {\tt projection block}. 

The default is {\tt num\_wann}.

\subsection[sel\_loc\_constrain]{\tt logical :: sel\_loc\_constrain}
If {\tt true} the centres of the objective Wannier Functions are constrained to either the centres of the first {\tt sel\_loc\_num} orbitals in the projection block or to new positions specified in the {\tt centre\_constrains} block. A modified spread functional, $\Omega_c$, with the addition of a constrain, is minimized in this case, see Marianetti \textit{et al} \cite{Marianetti} for the details of the minimisation. 

The default is {\tt false}

\subsection[sel\_loc\_constrain\_lambda]{\tt real(kind=dp) :: sel\_loc\_constrain\_lambda}
Value of the Lagrange multiplier for the constraint $ \lambda \sum_{n=1}^{J'} \left(\overline{\mathbf{r}}_n - \mathbf{r}_{0n}\right)$, where $J'$ is {\tt sel\_loc\_num} and $\mathbf{r}_{0i}$ is the desired centre for the i-th objective function. 
