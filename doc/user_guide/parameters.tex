...
{\sc jprime} & I & The number of objective Wannier Functions for selective localization \\
{\sc constrain_centres} & L & To constrain the centres of the OWFs \\
{\sc constrain_centres_tol} & R & The convergence tolerance for the constrained $\Omega$ \\
\hline
\end{tabular}
...
The block below shows all possible ways of setting the constraints. In particular, assuming J'$=5<$J :
...
\begin{itemize}
\item The first line sets the constraint for the centre of Wannier function number 2 (as defined in the projection block) to (0.0,0.0,0.0) in fractional co-ordinates.
\item The second line sets the constraint for the centre of Wannier function number 4 (as defined in the projection block) to (0.25,0.0,0.0) in fractional co-ordinates.
...
\begin{itemize}
\item[{\tt isolated}] $f(\epsilon_{n\mathbf{k}})$ is the identity matrix $I_{n\mathbf{k}}$
\item[{\tt erfc}] The occupation number matrix is given by: $$f(\epsilon_{n\mathbf{k}})=\frac{1}{2}\mathtt{ERFC}\left(\frac{\epsilon_{n\mathbf{k}} - \mu}{\sigma}\right) $$
\item[{\tt gaussian}] The occupation number matrix is given by $$f(\epsilon_{n\mathbf{k}})=\mathtt{EXP}\left(-\frac{(\epsilon_{n\mathbf{k}}-\mu)^2}{\sigma^2}\right)$$
\end{itemize}
...
\subsection[jprime]{\tt integer :: jprime}
The number of objective Wannier Functions, see \cite{Marianetti} for definition and use.

The default is {\tt num\_wann}

\subsection[constrain_centres]{\tt logical :: constrain_centres}
If \tt{true} the centres of the objective Wannier Functions are constrained to either the centres of the projection functions or to new positions specified in the \tt{centre_constrains} block. A modified spread functional, $\Omega_c$, with the addition of a penalty function, is minimized in this case, see Marianetti \textit{et al} \cite{Marianetti}. 

The default is {\tt false}

\subsection[constrain_centres_tol]{\tt real(kind=dp) :: constrain_centres_tol}
In the constrained centres approach the minimisation is said to be converged when either the change in $\Omega_c$ over {\tt
  conv\_window} successive iterations is less than {\tt
  conv\_tol} (these keywords must be defined in the input file) or the change in the target WF centres is less than {\tt constrain_centres_tol}. If these two criterion are not satisfied the algorithm ends after {\tt num_iter} iterations.

The default is {\tt 1.0E-9}.
