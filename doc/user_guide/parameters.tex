...
or read a Hamiltonian matrix from an external file
{\tt seedname\_htB.dat}.
...
Number of rows is equal to \verb#tran_num_ll#.
 

The default value is 0.

\subsection[tran\_num\_cr]{\tt integer :: tran\_num\_cr}
Number of rows in a conductor\_rigt-lead Hamiltonian matrix.
Number of columns is equal to \verb#tran_num_rr#.
...
{\tt seedname\_htR.dat} is not required.

The default value is {\tt TRUE}.

\subsection[translation\_centre\_frac]{\tt real(kind=dp) :: translation\_centre\_frac(3)}

Centre of the unit cell to which the final Wannier centers are translated. Numbers are in fractional coordinates with respect to the lattice vectors.
...
The largest distance between WF for which
...
The vector connecting two WF may be projected
...
Confined direction in a two-dimensional system
can be specified through this parameter.
