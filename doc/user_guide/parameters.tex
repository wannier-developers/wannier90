...
physical value, L for a logical value and S for a text string.
{\footnotesize
* {\sc write\_xyz } is only effective if {\sc translate\_home\_cell } is
\texttt{true} }} 
...
If \verb#num_shells#$>0$, then the number of shells to include in the finite difference expression.
If \verb#num_shells#$=0$, then the code will choose the shells automatically. 
...
unit cell at the end of the calculation. Useful for molecular systems
in which the molecule resides entirely within the home unit
cell. 
...
formatted file, \verb#seedname_centres.xyz#, for subsequent
visualisation. Only effective when {\sc translate\_home\_cell} is
\verb#true#. 
...
Restriction: $0.0<${\tt dis\_mix\_ratio}$\geq1.0$ . The default value is 0.5 
...
$\langle m|r^2|n\rangle$ (where $m$ and $n$ refer to Wannier
...
Dimension of the `super-unit-cell' in which the Wannier functions are
...
\subsection[wannier\_plot\_list]{\tt integer :: wannier\_plot\_list(:)}
 ...
 A list of Wannier functions to plot. The Wannier functions numbered
 as per the {\tt seedname.wout} file after the minimisation of the
 spread. 

 The default behaviour is to plot all Wannier functions. For example,
 to plot Wannier functions 4, 5, 6 and 10:
...
\wannier, it is a useful parameter to set as
it will be written driectly into the bxsf file.
