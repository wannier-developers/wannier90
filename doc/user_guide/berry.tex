...
We can use Wannier interpolation technique to efficiently calculate this matrix, and there are two derivation according to the degree of approximation. The method by Qiao is, using $1\approx\sum_{ l\in {\rm Wannier\,bands}}|l\bm{k}\rangle \langle l\bm{k}|$, calculate the matrix as
\begin{equation}
...
\end{equation}
while the other method by Ryoo is, without that identity matrix,
\begin{equation}
...
\end{equation}
Qiao's method does not require further matrix elements evaluated by {\tt pw2wannier90}, but it assumes an approximation since usually the number of Wannier bands is less than the number of Bloch bands. You can choose which method to evaluate this value with {\tt shc\_method} in the input file. For a full derivation please refer to Ref.~\cite{qiao-prb2018} or Ref.~\cite{ryoo-prb2019}.
