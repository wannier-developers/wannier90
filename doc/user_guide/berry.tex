...
\end{equation}
while the other method by Ryoo substitutes (... to be explained) Qiao's method does not require further matrix elements evaluated by {\tt pw2wannier90}, but it assumes a stronger approximation since usually the number of Wannier bands is less than the number of Bloch bands. You can choose which method to evaluate this value with {\tt shc\_method} in the input file. For a full derivation please refer to Ref.~\cite{qiao-prb2018} or Ref.~\cite{ryoo-prb2019}.
