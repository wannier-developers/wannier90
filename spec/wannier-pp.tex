...
This is a description of the how to use the wannier code as a
post-processing tool. 

The code must be run twice. On the first pass the logical keyword
\verb#postproc_setup# must be set to \verb#.true.# in the input file
\verb#seedname.win#.  Running the code then generates the file
...
Eq.~(62), SMV\footnote{Souza, Marzari and Vanderbilt,
  \textit{Phys.~Rev.~B} \textbf{65}, 035109 (2001)} Eq.~(22)). 

Once the overlaps and projection have been computed and written to
files \verb#seedname.mmn# and \verb#seedname.amn#, respectively, set
\verb#postproc_setup# to \verb#.false.# and run the code. Output is
...
determine the required overlap elements $M_{mn}^{(\mathbf{k,b})}$ (MV
Eq.~(25)) and projections $A_{mn}^{(\mathbf{k})}$ (MV Eq.~(62), SMV
Eq.~(22)), ie, \verb#M_matrix_orig# and \verb#A_matrix#, described in
...
\verb#n_proj#: integer; the number of projection centers, equal to the
number of Wannier functions \verb#num_wann#.

\verb#center#: three real numbers; projection function center
...
localised projection function. Units are in Angstrom.
...
specified, and the subsquent lines provide the projection center, the
...
in the list of $k$-points in the master input file. The real numbers
...
to the $k$-point in the list of $k$-points in the master input file,
...
\begin{enumerate}
\item Run an `scf' calculation with \verb#pw#
...
\item Run \verb#pw2wannier90#. First it reads \verb#pw2wannier90.in#,
  ...
  files specified by \verb#prefix#. Then generate 
  \verb#seedname.mmn#, \verb#seedname.amn#, \verb#seedname.eig#
\item Run \verb#wannier90# with \verb#postproc_setup#~=~\verb#.false.# to
  disentangle and localise Wannier functions
\end{enumerate}
