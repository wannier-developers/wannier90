...
if data.num_wann == data.num_bands:
    ...
    wan90.w90_helper_types.set_m_matrix_local(w90data, m_matrix)
else:
    m_matrix_loc = numpy.zeros((data.num_wann, data.num_wann, data.kmesh_info.nntot, data.num_kpts), dtype=numpy.cdouble, order='F')
...
#status = wan90.w90_helper_types.checkpoint(data, w90data, "postwann", ftn_output, comm)

status = wan90.w90_helper_types.plot_files(data, w90data, ftn_output, comm)

#status = wan90.w90_helper_types.transport(data, w90data, ftn_output, comm)
