...
contains
  ...
  subroutine wann_main(num_wann, param_wannierise, kmesh_info, param_input, u_matrix, m_matrix, &
    ...
    !integer, intent(in) :: num_cg_steps, num_iter
...
!           m_matrix (:,:,:, nkp)
    ...
    call comms_scatterv(m_matrix_loc, num_wann*num_wann*kmesh_info%nntot*counts(my_node_id), &
                        ...
                        num_wann*num_wann*kmesh_info%nntot*displs, stdout, seedname)
    ...
    if (param_wannierise%guiding_centres) then
      do n = 1, num_proj
        call utility_frac_to_cart(param_wannierise%proj_site(:, n), rguide(:, n), real_lattice)
      enddo
...
!       end if
    ...
    if (param_wannierise%selective_loc .and. param_wannierise%slwf_constrain) then
      lambda_loc = param_wannierise%slwf_lambda
    end if
    ...
    call wann_omega(csheet, sheet, rave, r2ave, rave2, wann_spread, num_wann, kmesh_info, &
                    ...
                    m_matrix_loc, lambda_loc, first_pass, stdout, seedname)

    ! public variables
    if (.not. param_wannierise%selective_loc) then
      param_wannierise%omega_total = wann_spread%om_tot
      param_input%omega_invariant = wann_spread%om_i
      param_wannierise%omega_tilde = wann_spread%om_d + wann_spread%om_od
    else
      param_wannierise%omega_total = wann_spread%om_tot
    ...
    if (param_wannierise%lfixstep) lquad = .false.
    ncg = 0
    iter = 0
    ...
    if (on_root) then
      ...
      if (param_wannierise%selective_loc .and. param_wannierise%slwf_constrain) then
        ...
        write (stdout, '(1x,a78)') repeat('-', 78)
      elseif (param_wannierise%selective_loc .and. .not. param_wannierise%slwf_constrain) then
        write (stdout, '(1x,i6,2x,E12.3,2x,F15.10,2x,F18.10,3x,F8.2,2x,a)') iter, &
          (wann_spread%om_tot - old_spread%om_tot)*param_input%lenconfac**2, &
          sqrt(abs(gcnorm1))*param_input%lenconfac, &
          wann_spread%om_tot*param_input%lenconfac**2, io_wallclocktime(), '<-- CONV'
    ...
    if (.not. param_wannierise%lfixstep .and. param_input%optimisation <= 0) then
      page_unit = io_file_unit()
      open (unit=page_unit, status='scratch', form='unformatted')
    endif

    ! main iteration loop
    do iter = 1, param_wannierise%num_iter

      lprint = .false.
      if ((mod(iter, param_wannierise%num_print_cycles) .eq. 0) .or. (iter .eq. 1) &
          .or. (iter .eq. param_wannierise%num_iter)) lprint = .true.

      ldump = .false.
      if ((param_wannierise%num_dump_cycles .gt. 0) .and. &
          (mod(iter, param_wannierise%num_dump_cycles) .eq. 0)) ldump = .true.

      if (lprint .and. on_root) write (stdout, '(1x,a,i6)') 'Cycle: ', iter

      if (param_wannierise%guiding_centres .and. (iter .gt. param_wannierise%num_no_guide_iter) &
      ...
      if (lsitesymmetry .or. param_wannierise%precond) then
        call wann_domega(csheet, sheet, rave, num_wann, kmesh_info, num_kpts, param_wannierise, &
                         ...
                         cdodq_loc, lambda_loc, param_input%timing_level, stdout, seedname, sym, cdodq)
      else
        call wann_domega(csheet, sheet, rave, num_wann, kmesh_info, num_kpts, param_wannierise, &
                         lsitesymmetry, counts, displs, ln_tmp_loc, m_matrix_loc, rnkb_loc, &
      ...
      if (param_wannierise%lfixstep) then
        ...
        alphamin = param_wannierise%fixed_step

        ! or a parabolic line search
      else

        ! take trial step
        cdq_loc(:, :, :) = cdqkeep_loc(:, :, :)*(param_wannierise%trial_step/(4.0_dp*kmesh_info%wbtot))
...
!             write(page_unit)   m_matrix
        ...
        call internal_optimal_step(wann_spread, trial_spread, doda0, alphamin, falphamin, lquad, &
                                   lprint, param_wannierise%trial_step, param_input, stdout)
      endif

      ! print line search information
      if (lprint .and. param_input%iprint > 2 .and. on_root) then
        write (stdout, *) ' LINE --> Spread at initial point       :', &
          wann_spread%om_tot*param_input%lenconfac**2
        if (.not. param_wannierise%lfixstep) &
        ...
        if (.not. param_wannierise%lfixstep) then
          write (stdout, *) ' LINE --> Trial step length             :', param_wannierise%trial_step
          if (lquad) then
          ...
          write (stdout, *) ' LINE --> Fixed step length             :', param_wannierise%fixed_step
        endif
        write (stdout, *) ' LINE --> CG coefficient                :', gcfac
      endif

      ! if taking a fixed step or if parabolic line search was successful
      if (param_wannierise%lfixstep .or. lquad) then
...
!                read(page_unit)  m_matrix
      ...
      if (ldump) then
        ...
        call comms_gatherv(u_matrix_loc, num_wann*num_wann*counts(my_node_id), &
                           u_matrix, num_wann*num_wann*counts, num_wann*num_wann*displs, stdout, seedname)
        ...
        if (on_root) call param_write_chkpt('postdis', param_input, wann_data, kmesh_info, &
                                            ...
                                            real_lattice, recip_lattice, stdout, seedname)
      endif
      ...
      if (lconverged) then
        write (stdout, '(/13x,a,es10.3,a,i2,a)') &
          '<<<     Delta <', param_wannierise%conv_tol, &
          '  over ', param_wannierise%conv_window, ' iterations     >>>'
...
!    end do!nn
    ...
    if (param_wannierise%selective_loc .and. param_wannierise%slwf_constrain) then
      rnr0n2 = 0.0_dp
      do iw = 1, param_wannierise%slwf_num
        rnr0n2(iw) = (wann_data%centres(1, iw) - param_wannierise%ccentres_cart(iw, 1))**2 &
                     + (wann_data%centres(2, iw) - param_wannierise%ccentres_cart(iw, 2))**2 &
                     + (wann_data%centres(3, iw) - param_wannierise%ccentres_cart(iw, 3))**2
      end do
    end if
    ...
    if (on_root) then
      ...
      if (param_wannierise%selective_loc .and. param_wannierise%slwf_constrain) then
        ...
        write (stdout, '(1x,a78)') repeat('-', 78)
      else if (param_wannierise%selective_loc .and. .not. param_wannierise%slwf_constrain) then
        write (stdout, '(3x,a21,a,f15.9)') '     Spreads ('//trim(param_input%length_unit)//'^2)', &
...
!~    call internal_check_unitarity()
    call wann_check_unitarity(num_kpts, num_wann, u_matrix, param_input%timing_level, stdout, seedname)
...
!    if (write_r2mn) call wann_write_r2mn()
    ...
    if (param_input%have_disentangled .and. param_wannierise%write_proj) &
      call wann_calc_projection(num_bands, num_wann, num_kpts, u_matrix_opt, eigval, &
                                dis_data%lwindow, param_input%timing_level, stdout, seedname)
...
!    if (ierr/=0) call io_error('Error in deallocating m_matrix_1b_loc in wann_main')
    ...
    if (param_input%timing_level > 0 .and. on_root) call io_stopwatch('wann: main', 2, stdout, seedname)
...
!     integer, intent(in) :: conv_noise_num
      ...
      if (iter .le. param_wannierise%conv_window) then
      ...
      if (conv_count .lt. param_wannierise%conv_window) then
        return
      else
!~         write(stdout,*) (history(j),j=1,conv_window)
        do j = 1, param_wannierise%conv_window
          if (abs(history(j)) .gt. param_wannierise%conv_tol) return
        enddo
      endif

      if ((param_wannierise%conv_noise_amp .gt. 0.0_dp) .and. &
          (noise_count .lt. param_wannierise%conv_noise_num)) then
        if (lfirst) then
          lfirst = .false.
          ...
          if (abs(save_spread - wann_spread%om_tot) .lt. param_wannierise%conv_tol) then
            lconverged = .true.
            return
          else
            save_spread = wann_spread%om_tot
...
!         gcnorm1 = real(zdotc(num_kpts*num_wann*num_wann,cdodq_precond,1,cdodq,1),dp)
      ...
      call comms_allreduce(gcnorm1, 1, 'SUM', stdout, seedname)
      ...
      if (lrandom) then
        if (on_root) write (stdout, '(a,i3,a,i3,a)') &
          ...
          param_wannierise%conv_noise_num, ' ]'
        call internal_random_noise(param_wannierise%conv_noise_amp, num_wann, counts, cdq_loc, &
                                   stdout)
      endif
      ...
      call comms_allreduce(doda0, 1, 'SUM', stdout, seedname)
      ...
      if (doda0 .gt. 0.0_dp) then
        ...
        if (ncg .gt. 0) then
          ...
          cdq_loc(:, :, :) = cdodq_loc(:, :, :)
          if (lrandom) call internal_random_noise(param_wannierise%conv_noise_amp, num_wann, &
...
!     integer, intent(in) :: iprint, timing_level
      ...
      if (abs(eqa/(fac*wann_spread%om_tot)) .gt. epsilon(1.0_dp)) then
        ...
        if (lprint .and. param_input%iprint > 2 .and. on_root) write (stdout, *) &
...
!     integer, intent(in) :: nnlist(:, :)
      ...
      if (timing_level > 1 .and. on_root) call io_stopwatch('wann: main: u_and_m', 1, stdout, seedname)
...
!!$            call io_error('wann_main: problem in ZHEEV in internal_new_u_and_m')
      ...
      call comms_gatherv(cdq_loc, num_wann*num_wann*counts(my_node_id), cdq, &
                         num_wann*num_wann*counts, num_wann*num_wann*displs, stdout, seedname)
      call comms_bcast(cdq(1, 1, 1), num_wann*num_wann*num_kpts, stdout, seedname)
...
!~    end subroutine internal_svd_omega_i
  ...
  subroutine wann_phases(csheet, sheet, rguide, irguide, num_wann, kmesh_info, num_kpts, m_matrix, &
                         gamma_only, counts, displs, m_matrix_loc, rnkb, timing_level, stdout, seedname, m_w)
    !==================================================================!
...
!   real(kind=dp), intent(in) :: bka(:, :)
    ...
    if (timing_level > 1 .and. on_root) call io_stopwatch('wann: phases', 1, stdout, seedname)
    ...
    do loop_wann = 1, num_wann
      ...
      call comms_allreduce(csum(1), kmesh_info%nnh, 'SUM', stdout, seedname)
...
!    enddo
    ...
    if (timing_level > 1 .and. on_root) call io_stopwatch('wann: phases', 2, stdout, seedname)
...
!   integer, intent(in) :: timing_level
    ...
    if (param_input%timing_level > 1 .and. on_root) call io_stopwatch('wann: omega', 1, stdout, seedname)
    ...
    call comms_allreduce(rave(1, 1), num_wann*3, 'SUM', stdout, seedname)
...
!~    enddo
    ...
    call comms_allreduce(r2ave(1), num_wann, 'SUM', stdout, seedname)
...
!~    enddo
    ...
    if (param_wannierise%selective_loc) then
      ...
      do nkp_loc = 1, counts(my_node_id)
        do nn = 1, kmesh_info%nntot
          summ = 0.0_dp
          do n = 1, param_wannierise%slwf_num
            summ = summ &
          ...
          wann_spread%om_iod = wann_spread%om_iod &
                               + kmesh_info%wb(nn)*(real(param_wannierise%slwf_num, dp) - summ)
        enddo
      enddo

      call comms_allreduce(wann_spread%om_iod, 1, 'SUM', stdout, seedname)
      ...
      call comms_allreduce(wann_spread%om_d, 1, 'SUM', stdout, seedname)
          ...
          do nn = 1, kmesh_info%nntot
            do n = 1, param_wannierise%slwf_num
              wann_spread%om_nu = wann_spread%om_nu + 2.0_dp*kmesh_info%wb(nn)* &
                                  ...
                                  sum(kmesh_info%bk(:, nn, nkp)*param_wannierise%ccentres_cart(n, :))
            enddo
          enddo
        enddo

        call comms_allreduce(wann_spread%om_nu, 1, 'SUM', stdout, seedname)

        wann_spread%om_nu = wann_spread%om_nu/real(num_kpts, dp)

        do n = 1, param_wannierise%slwf_num
          wann_spread%om_nu = wann_spread%om_nu &
                              + lambda_loc*sum(param_wannierise%ccentres_cart(n, :)**2)
        end do
      ...
      if (first_pass) then
        ...
        call comms_allreduce(wann_spread%om_i, 1, 'SUM', stdout, seedname)

        wann_spread%om_i = wann_spread%om_i/real(num_kpts, dp)
        first_pass = .false.
      else
        wann_spread%om_i = param_input%omega_invariant
      endif
      ...
      call comms_allreduce(wann_spread%om_od, 1, 'SUM', stdout, seedname)
    ...
    if (param_input%timing_level > 1 .and. on_root) call io_stopwatch('wann: omega', 2, stdout, seedname)
  ...
  subroutine wann_domega(csheet, sheet, rave, num_wann, kmesh_info, num_kpts, param_wannierise, &
                         lsitesymmetry, counts, displs, ln_tmp_loc, m_matrix_loc, rnkb_loc, &
                         cdodq_loc, lambda_loc, timing_level, stdout, seedname, sym, cdodq)
    !==================================================================!
    !                                                                  !
...
!   real(kind=dp), intent(in) :: ccentres_cart(:, :)
    ...
    if (timing_level > 1 .and. on_root) call io_stopwatch('wann: domega', 1, stdout, seedname)
        ...
        do nn = 1, kmesh_info%nntot
          do n = 1, num_wann
            r0kb(n, nn, nkp_loc) = sum(kmesh_info%bk(:, nn, nkp) &
                                       *param_wannierise%ccentres_cart(n, :))
          enddo
        enddo
      enddo
    end if
    ...
    do nkp_loc = 1, counts(my_node_id)
      ...
      do nn = 1, kmesh_info%nntot
        ...
        if (param_wannierise%selective_loc) then
          do n = 1, num_wann
            do m = 1, num_wann
              if (m <= param_wannierise%slwf_num) then
                if (n <= param_wannierise%slwf_num) then
                  ! A[R^{k,b}]=(R-Rdag)/2
              ...
              else if (n <= param_wannierise%slwf_num) then
                cdodq_loc(m, n, nkp_loc) = cdodq_loc(m, n, nkp_loc) &
    ...
    if (present(cdodq)) then
      ...
      call comms_gatherv(cdodq_loc, num_wann*num_wann*counts(my_node_id), &
                         cdodq, num_wann*num_wann*counts, num_wann*num_wann*displs, stdout, seedname)
      call comms_bcast(cdodq(1, 1, 1), num_wann*num_wann*num_kpts, stdout, seedname)
      if (lsitesymmetry) then
    ...
    if (timing_level > 1 .and. on_root) call io_stopwatch('wann: domega', 2, stdout, seedname)
...
!~    copy%om_3   =  orig%om_3
  ...
  subroutine wann_calc_projection(num_bands, num_wann, num_kpts, u_matrix_opt, eigval, lwindow, &
    ...
    if (timing_level > 1 .and. on_root) call io_stopwatch('wann: calc_projection', 1, stdout, seedname)
    ...
    if (timing_level > 1 .and. on_root) call io_stopwatch('wann: calc_projection', 2, stdout, seedname)
...
!~    if (ierr/=0) call io_error('Error in deallocating f_w2 in wann_write_vdw_data')
  ...
  !========================================!
  subroutine wann_check_unitarity(num_kpts, num_wann, u_matrix, timing_level, stdout, seedname)
    !========================================!
    ...
    if (timing_level > 1 .and. on_root) call io_stopwatch('wann: check_unitarity', 1, stdout, seedname)
    ...
    if (timing_level > 1 .and. on_root) call io_stopwatch('wann: check_unitarity', 2, stdout, seedname)
...
!   integer, intent(in) :: timing_level
    ...
    if (param_input%timing_level > 1 .and. on_root) call io_stopwatch('wann: svd_omega_i', 1, stdout, seedname)
    ...
    if (param_input%timing_level > 1 .and. on_root) call io_stopwatch('wann: svd_omega_i', 2, stdout, seedname)
  ...
  subroutine wann_main_gamma(num_wann, param_wannierise, kmesh_info, &
                             ...
                             dis_data, mp_grid, stdout, seedname)
    !==================================================================!
    !                                                                  !
...
!~    lguide = .false.
    ...
    if (kmesh_info%nntot .eq. 3) param_wannierise%guiding_centres = .false.
...
!~    endif
    if (param_wannierise%guiding_centres .and. (param_wannierise%num_no_guide_iter .le. 0)) then
      call wann_phases(csheet, sheet, rguide, irguide, num_wann, kmesh_info, &
                       ...
                       m_matrix_loc, rnkb, param_input%timing_level, stdout, seedname)
      irguide = 1
    endif
    ...
    do iter = 1, param_wannierise%num_iter
      ...
      ldump = .false.
      if ((param_wannierise%num_dump_cycles .gt. 0) .and. (mod(iter, param_wannierise%num_dump_cycles) .eq. 0)) ldump = .true.
...
!~       endif
      ...
      if (param_wannierise%guiding_centres .and. (iter .gt. param_wannierise%num_no_guide_iter) &
        ...
        call wann_phases(csheet, sheet, rguide, irguide, num_wann, kmesh_info, &
                         num_kpts, m_matrix, .true., counts, displs, &
                         m_matrix_loc, rnkb, param_input%timing_level, stdout, seedname, m_w)
        irguide = 1
      endif
...
!~    if (iprint>2) call internal_svd_omega_i()
    if (param_input%iprint > 2) then
      call wann_svd_omega_i(num_wann, num_kpts, kmesh_info, m_matrix, param_input, &
                            stdout, seedname)
    endif
...
!~    if (write_r2mn) call internal_write_r2mn()
    ...
    if (param_input%have_disentangled .and. param_wannierise%write_proj) &
      call wann_calc_projection(num_bands, num_wann, num_kpts, u_matrix_opt, &
                                eigval, lwindow, param_input%timing_level, stdout, seedname)
