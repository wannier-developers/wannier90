...
module w90_kslice
  ...
  subroutine k_slice
    ...
    use w90_parameters, only   : num_wann,kslice,kslice_task,&
                                 ...
                                 kslice_cntr_energy,found_kslice_cntr_energy,&
    ...
    integer           :: loop_tot,loop_x,loop_y,n,n1,n2,n3
    ...
    if(plot_curv_heatmap .and. plot_morb_heatmap) call io_error(&
    ...
    if(on_root) then
      ...
      ! Find the reciprocal of bvec, store in avec
       ...
       write(stdout,'(/,/,1x,a)') '==============================='
       write(stdout,'(1x,a)')     'Plotting on a slice in k-space:'
       write(stdout,'(1x,a)')     '==============================='
 ...
 'Need to specify either "fermi_energy" or "num_elec_cell" when plot_curv_heatmap=T'&
       ...
       elseif(plot_morb_heatmap) then
          ! LC_tilde-mu.Omega, IC_tilde-mu.Omega, and their sum M_orb(k)
          write(stdout,'(/,3x,a)') '* Orbital magnetization M_orb(k) in a.u.'
          if(.not.found_fermi_energy) call io_error&
               (&
'Need to set either "fermi_energy" or "num_elec_cell" when plot_morb_heatmap=T'&
       ...
       !
!       do loop_tot=0,kslice_num_points**2-1
!          loop_x=loop_tot/kslice_num_points
!          loop_y=loop_tot-loop_x*kslice_num_points
       do loop_tot=0,product(kslice_interp_mesh)-1
          ...
          if(plot_curv_heatmap) then
             ...
             ! To make a plot as in Fig. 3 of Yao et al, PRL 92, 037204 (2004)
             !
             ...
             if(curv_au>0) then
                if(curv_au>10) then
                   ...
                   curv_au=0.5_dp ! 1.0 would work as well
                endif
             else
                if(curv_au<-10) then
                   curv_au=-log10(-curv_au)
                else
                   curv_au=-0.5_dp ! -0.99 would work as well, but not -1.0!
                endif
             endif
...
!             write(bandsunit,'(99E16.8)') eig
       ...
       ! TO DO: Add here scripts for python and octave, allow to choose
       !        between them (uncomment kslice_plot_format in parameters.F90)
       if(plot_energy_cntr) then
          scriptunit=io_file_unit()
          open(scriptunit,file=trim(seedname)//'_energy_cntr.gnu',form='formatted')
          ...
          enddo
          write(scriptunit,'(a)') '#Uncomment next two lines to create postscript'
          write(scriptunit,'(a)') '#set term post eps enh'
          write(scriptunit,'(a)') '#set output "'//trim(seedname)//'_econtour.eps"'
          ...
          if(num_wann==1) then
             write(scriptunit,'(a)') 'plot "xyz_001.dat" using 1:2 w lines ls 1'
          else
             write(scriptunit,'(a)') 'plot "xyz_001.dat" using 1:2 w lines ls 1,'&
                  //achar(92)
          endif
       ...
       if(plot_curv_heatmap .or. plot_morb_heatmap) then
          scriptunit=io_file_unit()
          if(plot_curv_heatmap) then
           open(scriptunit,file=trim(seedname)//'_curv_heatmap.py',form='formatted')
        elseif(plot_morb_heatmap) then
           open(scriptunit,file=trim(seedname)//'_morb_heatmap.py',form='formatted')
        endif
        write(scriptunit,'(a)') 'import pylab as pl'
        write(scriptunit,'(a)') 'import numpy as np'
        write(scriptunit,'(a)') 'import mpl_toolkits'
        write(scriptunit,'(a)') ' '
        write(scriptunit,'(a)') "outfile = '"//trim(seedname)//"_curv_heatmap.pdf'"
        write(scriptunit,'(a)') ' '
        write(scriptunit,'(a)') "x = np.loadtxt('"//trim(seedname)//"_slice_x.dat')"
        write(scriptunit,'(a)') 'dimx=x.size'
        write(scriptunit,'(a)') ' '
        write(scriptunit,'(a)') "y = np.loadtxt('"//trim(seedname)//"_slice_y.dat')"
        write(scriptunit,'(a)') 'dimy=y.size'
        write(scriptunit,'(a)') ' '
        if(plot_energy_cntr) then
           write(scriptunit,'(a)')&
                '# Energy level for isocontours (typically Fermi level)'
           write(scriptunit,'(a,f12.6)') 'ef=',kslice_cntr_energy
           write(scriptunit,'(a)') ' '
           write(scriptunit,'(a)')&
                "bands=np.loadtxt('"//trim(seedname)//"_slice_bands.dat')"
           write(scriptunit,'(a)') 'numbands=bands.size/dimx/dimy'
           write(scriptunit,'(a)') 'bands2=bands.reshape((dimx,dimy,numbands))'
        ...
        if(plot_curv_heatmap) then
           write(scriptunit,'(a)')&
                "z = np.loadtxt('"//trim(seedname)//"_slice_curv.dat')"
        elseif(plot_morb_heatmap) then
           write(scriptunit,'(a)')&
                "z = np.loadtxt('"//trim(seedname)//"_slice_morb.dat')"
        endif
