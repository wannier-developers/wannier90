...
program postw90
  ...
  if (pw90_calcs%dos .and. index(dos_data%task, 'dos_plot') > 0) then
    call dos_main(num_bands, num_kpts, nrpts, irvec, ndegen, rpt_origin, eigval, real_lattice, &
                  crvec, u_matrix, v_matrix, dis_data, k_points, stdout, seedname, num_wann, &
                  param_input, dos_data, pw90_common, berry, postw90_oper, comm, pw90_ham)
  end if
...
!  if(dos .and. index(dos_task,'find_fermi_energy')>0) call find_fermi_level
  ...
  if (pw90_calcs%kpath) then
    call k_path(nrpts, num_bands, num_kpts, irvec, eigval, v_matrix, dis_data, kmesh_info, &
                k_points, param_input, pw90_common, postw90_oper, crvec, ndegen, rpt_origin, &
                real_lattice, u_matrix, num_wann, spec_points, fermi, berry, spin_hall, kpath, &
                pw90_spin, pw90_ham, recip_lattice, physics%bohr, stdout, seedname, comm)
  end if
  ! ---------------------------------------------------------------------------
  ...
  if (pw90_calcs%kslice) then
    call k_slice(real_lattice, nrpts, num_bands, num_kpts, num_wann, irvec, ndegen, rpt_origin, &
                 eigval, v_matrix, u_matrix, dis_data, kmesh_info, k_points, param_input, &
                 pw90_common, postw90_oper, spin_hall, comm, recip_lattice, crvec, physics%bohr, &
                 stdout, seedname)
  end if
  ! --------------------
  ...
  if (pw90_common%spin_moment) then
    call spin_get_moment(num_bands, num_kpts, nrpts, irvec, ndegen, rpt_origin, eigval, &
                         real_lattice, crvec, u_matrix, v_matrix, dis_data, k_points, param_input, &
                         pw90_common, postw90_oper, pw90_spin, berry%wanint_kpoint_file, fermi, &
                         num_wann, param_input%iprint, stdout, seedname, comm)
  end if
  ! -------------------------------------------------------------------
  ...
  if (pw90_calcs%berry) then
    call berry_main(num_bands, num_kpts, nrpts, irvec, eigval, u_matrix, v_matrix, dis_data, &
                    kmesh_info, k_points, crvec, ndegen, real_lattice, rpt_origin, postw90_oper, &
                    param_input, fermi, num_wann, berry, pw90_common, pw90_spin, spin_hall, &
                    pw90_ham, physics, stdout, seedname, comm, int_kpts, num_int_kpts_on_node, &
                    weight, cell_volume)
  end if
  ! -----------------------------------------------------------------
  ...
  if (pw90_calcs%boltzwann) then
    call boltzwann_main(real_lattice, nrpts, num_bands, num_kpts, irvec, ndegen, rpt_origin, &
                        eigval, v_matrix, u_matrix, k_points, postw90_oper, crvec, dis_data, &
                        param_input, num_wann, boltz, pw90_common, pw90_spin, pw90_ham, physics, &
                        stdout, seedname, comm, cell_volume, dos_data)
  end if

  if (pw90_calcs%gyrotropic) then
    call gyrotropic_main(num_bands, num_kpts, num_wann, nrpts, irvec, ndegen, rpt_origin, eigval, &
                         real_lattice, crvec, u_matrix, v_matrix, dis_data, kmesh_info, k_points, &
                         param_input, pw90_common, postw90_oper, pw90_ham, comm, physics, stdout, &
                         seedname, gyrotropic, berry, fermi, cell_volume)
  endif
