...
contains
  ...
  subroutine boltzwann_main(real_lattice, nrpts, num_bands, num_kpts, irvec, ndegen, rpt_origin, &
                            eigval, v_matrix, u_matrix, k_points, postw90_oper, &
                            crvec, &
                            dis_data, param_input, num_wann, boltz, pw90_common, pw90_spin, &
                            pw90_ham, physics, stdout, seedname, comm, cell_volume, dos_data)
    ...
    use pw90_parameters, only: boltzwann_type, postw90_common_type, postw90_spin_type, postw90_ham_type, dos_plot_type, postw90_oper_type, spin_hall_type
    ...
    ! arguments
    integer, intent(in) :: nrpts, num_bands, num_kpts, num_wann
    ...
    call calcTDFandDOS(real_lattice, nrpts, num_bands, num_kpts, irvec, ndegen, rpt_origin, &
                       eigval, v_matrix, u_matrix, dis_data, k_points, postw90_oper, &
                       crvec, &
                       TDF, TDFEnergyArray, num_wann, param_input, boltz, pw90_spin, &
                       pw90_common%spin_decomp, pw90_ham, stdout, seedname, comm, cell_volume, dos_data, pw90_common)
    ...
    do LocalIdx = 1, counts(my_node_id)
      ...
      LocalElCond(:, LocalIdx) = sum(IntegrandArray, DIM=2)*boltz%tdf_energy_step
      ...
      LocalSigmaS(:, LocalIdx) = sum(IntegrandArray, DIM=2)*boltz%tdf_energy_step/TempArray(TempIdx)
      do j = 1, 3
        do i = 1, j
      ...
      LocalKappa(:, LocalIdx) = sum(IntegrandArray, DIM=2)*boltz%tdf_energy_step/TempArray(TempIdx)
...
104 FORMAT(11G18.10)
  ...
  subroutine calcTDFandDOS(real_lattice, nrpts, num_bands, num_kpts, irvec, ndegen, rpt_origin, &
                           eigval, v_matrix, u_matrix, dis_data, k_points, postw90_oper, &
                           crvec, &
                           TDF, TDFEnergyArray, num_wann, param_input, boltz, pw90_spin, &
                           spin_decomp, pw90_ham, stdout, seedname, comm, cell_volume, dos_data, pw90_common)
    ...
    !!
    use pw90_parameters, only: boltzwann_type, postw90_spin_type, postw90_ham_type, dos_plot_type, postw90_common_type, postw90_oper_type, spin_hall_type
    use w90_comms, only: comms_bcast, w90commtype, mpirank
    use w90_constants, only: dp, cmplx_0, cmplx_i
    ...
    if (on_root .and. (param_input%iprint > 1)) then
      if (boltz%TDF_smr_fixed_en_width/(TDFEnergyArray(2) - TDFEnergyArray(1)) < min_smearing_binwidth_ratio) then
        ...
        write (stdout, '(7X,A,G18.10)') &
          trim(param_get_smearing_type(boltz%TDF_smr_index))//", non-adaptive, width (eV) =", &
          boltz%TDF_smr_fixed_en_width
      end if
    end if
            ...
            do i = -1, 1, 2
              do j = -1, 1, 2
                do k = -1, 1, 2
                  ...
                  call wham_get_eig_deleig(num_bands, num_kpts, nrpts, irvec, ndegen, rpt_origin, eigval, &
                                      real_lattice, crvec, u_matrix, v_matrix, dis_data, k_points, param_input, pw90_common, comm, &
                                           kpt, eig, del_eig, HH, delHH, UU, num_wann, pw90_ham, &
                                           stdout, seedname)
                  call dos_get_levelspacing(del_eig, boltz%kmesh, levelspacing_k, num_wann)
  ...
  subroutine TDF_kpt(kpt, EnergyArray, eig_k, deleig_k, TDF_k, num_wann, param_input, boltz, &
    ...
    do BandIdx = 1, num_wann
      ...
      smear = boltz%TDF_smr_fixed_en_width
      ...
      do loop_f = min_f, max_f
        if (DoSmearing) then
          ...
          rdum = utility_w0gauss(arg, boltz%TDF_smr_index, stdout, seedname)/smear
        else
