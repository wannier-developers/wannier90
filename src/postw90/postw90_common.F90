...
!    call comms_bcast(num_species,1) ! Ivo: not used in postw90, right?
    ...
    call comms_bcast(dos_data%adpt_smr_max, 1, stdout, seedname, world)
    call comms_bcast(dos_data%smr_fixed_en_width, 1, stdout, seedname, world)
    call comms_bcast(dos_data%adpt_smr_fac, 1, stdout, seedname, world)
...
! Tsirkin
    ...
    call comms_bcast(berry%kubo_adpt_smr, 1, stdout, seedname, world)
    call comms_bcast(berry%kubo_adpt_smr_fac, 1, stdout, seedname, world)
    call comms_bcast(berry%kubo_adpt_smr_max, 1, stdout, seedname, world)
    call comms_bcast(berry%kubo_smr_fixed_en_width, 1, stdout, seedname, world)
    call comms_bcast(berry%kubo_smr_index, 1, stdout, seedname, world)
...
! ----------------------------------------------
    ...
    call comms_bcast(boltz%dos_adpt_smr, 1, stdout, seedname, world)
    call comms_bcast(boltz%dos_smr_fixed_en_width, 1, stdout, seedname, world)
    call comms_bcast(boltz%dos_adpt_smr_fac, 1, stdout, seedname, world)
    call comms_bcast(boltz%dos_adpt_smr_max, 1, stdout, seedname, world)
    ...
    call comms_bcast(boltz%dos_smr_index, 1, stdout, seedname, world)
...
! [lp] Shift the WF to have the minimum distance IJ, see also ws_distance.F90
  ...
  subroutine pw90common_fourier_R_to_k_new(kpt, OO_R, num_wann, param_input, wann_data, &
                                           ...
                                           crdist_ws, wdist_ndeg, stdout, seedname, OO, OO_dx, &
                                           OO_dy, OO_dz)
    !=======================================================!
    !                                                       !
    !! For OO:
...
! [lp] Original code, without IJ-dependent shift:
  ...
  subroutine pw90common_fourier_R_to_k_new_second_d_TB_conv(kpt, OO_R, oo_a_R, num_wann, &
                                                            ...
                                                            recip_lattice, mp_grid, irdist_ws, &
                                                            crdist_ws, wdist_ndeg, stdout, &
                                                            seedname, OO, OO_da, OO_dadb)
    !=======================================================!
