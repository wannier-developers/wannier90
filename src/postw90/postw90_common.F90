...
!                               irvec, force_recompute)
    ...
    if (param_input%use_ws_distance) then
      CALL ws_translate_dist(param_input%ws_distance_tol, param_input%ws_search_size, &
                             ...
                             param_input%iprint, mp_grid, nrpts, irvec, force_recompute=.true.)
    endif
...
!           cycle
  ...
  subroutine wigner_seitz(count_pts)
    ...
    use w90_parameters, only: mp_grid, real_lattice, param_input, recip_lattice
    ...
    real(kind=dp) :: real_metric(3, 3), recip_metric(3, 3) !JJ for call to utility_metric
