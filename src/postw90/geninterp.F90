...
!   local variables
  ...
  subroutine geninterp_main(real_lattice, recip_lattice, nrpts, num_bands, num_kpts, num_wann, &
                            ...
                            crvec, comm, dis_data, pw90_common, stdout, seedname, geninterp, &
                            pw90_ham, param_input)
    ...
    do i = 1, counts(my_node_id)
      ...
      if (geninterp%alsofirstder) then
        call wham_get_eig_deleig(num_bands, num_kpts, nrpts, irvec, ndegen, rpt_origin, eigval, &
                                 ...
                                 param_input, pw90_common, comm, kpt, localeig(:, i), &
                                 localdeleig(:, :, i), HH, delHH, UU, num_wann, pw90_ham, stdout, &
                                 seedname)
      else
