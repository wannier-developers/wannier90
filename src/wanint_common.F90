...
!==============================================================================
  ...
  subroutine wanint_setup
    ...
    use w90_parameters, only  : real_lattice!optics_num_points, optics_adaptive_pts,

    integer        :: ierr,i,j,k,ikpt,loop_rpt
...
!  allocate(adkpt(3,optics_adaptive_pts**3),stat=ierr)
!  if (ierr/=0) call io_error('Error in allocating adkpt in wanint_setup')
!
!  ikpt=0
!   do i = - (optics_adaptive_pts-1)/2, (optics_adaptive_pts-1)/2
!     do j = - (optics_adaptive_pts-1)/2, (optics_adaptive_pts-1)/2
!        do k = - (optics_adaptive_pts-1)/2, (optics_adaptive_pts-1)/2
!           ikpt = ikpt+1 
!           adkpt (1, ikpt) = real(i,dp) / (optics_num_points*optics_adaptive_pts)
!           adkpt (2, ikpt) = real(j,dp) / (optics_num_points*optics_adaptive_pts)
!           adkpt (3, ikpt) = real(k,dp) / (optics_num_points*optics_adaptive_pts)
!        end do
!      end do
!   end do
  ...
  do loop_rpt=1,nrpts
     ...
     ! hence the transpose
     !
     crvec(:,loop_rpt)=matmul(transpose(real_lattice),irvec(:,loop_rpt))
  end do
  ...
  !===========================================================!
  !                                                           !
  ! read kpoints from kpoints.dat and distribute              !
  !                                                           !
  !===========================================================!
...
106 call io_error('Error: Problem opening file kpoint.dat in wanint_get_kpoint_file')
  ...
  !===========================================================!
    ...
    call comms_bcast(automatic_translation,1)
    ...
    call comms_bcast(tran_easy_fix,1) ! a boolean that tells the code to use the "easy_fix" method for fixing the WF parities
...
call comms_bcast(one_dim_dir,1)
...
!    call comms_bcast(smear_temp,1) 
!    call comms_bcast(eps_occ,1) 
!    call comms_bcast(alpha,1) 
!    call comms_bcast(beta,1) 
!    call comms_bcast(phase_convention,len(phase_convention)) 
!    call comms_bcast(adpt_smr_steps,1) 
!    call comms_bcast(dd_only,1) 
!    call comms_bcast(spin_purify_wfs,1)
...
! ----------------------------------------------
    ...
    ! 'eigval' 'kpt_latt' 'adpt_smr_width' and 'fermi_level' are different from
    ...
    if(.not.on_root) then
       ...
       if (ierr/=0) call io_error('Error allocating atoms_species_num in wanint_param_dist')
       allocate(atoms_label(num_species),stat=ierr)
       if (ierr/=0) call io_error('Error allocating atoms_label in wanint_param_dist')
       allocate(atoms_symbol(num_species),stat=ierr)
       if (ierr/=0) call io_error('Error allocating atoms_symbol in wanint_param_dist')
    end if
    call comms_bcast(atoms_species_num(1),num_species)
    call comms_bcast(atoms_label(1),num_species)
    call comms_bcast(atoms_symbol(1),num_species)


    if(.not.on_root) then
    !max_sites=maxval(atoms_species_num)
    allocate(atoms_pos_frac(3,maxval(atoms_species_num),num_species),stat=ierr)
       if (ierr/=0) call io_error('Error allocating atoms_pos_frac in param_get_atoms')
    allocate(atoms_pos_cart(3,maxval(atoms_species_num),num_species),stat=ierr)
       if (ierr/=0) call io_error('Error allocating atoms_pos_cart in param_get_atoms')


       allocate(wannier_centres(3,num_wann),stat=ierr)
       if (ierr/=0) call io_error('Error allocating wannier_centres in wanint_param_dist')
...
!       allocate(fermi_level(-nfermi+1:nfermi-1),stat=ierr)
!       if (ierr/=0) call io_error&
!            ('Error allocating fermi_level in wanint_param_dist')
    end if
   call comms_bcast(wannier_centres(1,1),size(wannier_centres,2)*size(wannier_centres,1))
    call comms_bcast(eigval(1,1),num_bands*num_kpts)
    call comms_bcast(kpt_latt(1,1),3*num_kpts)
    call comms_bcast(atoms_pos_frac(1,1,1),3*maxval(atoms_species_num)*num_species)
...
!    call comms_bcast(fermi_level(-nfermi+1),2*nfermi-1)
...
!=======================================================================
  ...
  subroutine get_occupancies!(eig,occ,efermi)

!    use w90_constants, only     : dp,elem_charge_SI,k_B_SI
!    use w90_parameters, only    : smear_temp,num_wann !,fermi_energy
!
!    ! Arguments
!    !
...
!    else
!       !
!       ! Use a Fermi-Dirac occupancy (T=smear_temp)
!       !
...
!    end if
!
  end subroutine get_occupancies
...
!=======================================================================
  ...
  if (timing_level>1.and.on_root) call io_stopwatch('plot: wigner_seitz',1)
  ...
  if (timing_level>1.and.on_root) call io_stopwatch('plot: wigner_seitz',2)

  return  
end subroutine wigner_seitz

end module w90_wanint_common
