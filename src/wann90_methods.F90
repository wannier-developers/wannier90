...
!     io_stopwatch
    ...
    call comms_bcast(driver%postproc_setup, 1, stdout, seedname)
    call comms_bcast(w90_calcs%cp_pp, 1, stdout, seedname)
    ...
    call comms_bcast(energy_unit, 1, stdout, seedname)
    call comms_bcast(param_input%length_unit, 1, stdout, seedname)
    call comms_bcast(param_plot%wvfn_formatted, 1, stdout, seedname)
    ...
    call comms_bcast(param_plot%spin, 1, stdout, seedname)
    call comms_bcast(param_wannierise%num_dump_cycles, 1, stdout, seedname)
    call comms_bcast(param_wannierise%num_print_cycles, 1, stdout, seedname)
    call comms_bcast(atoms%num_atoms, 1, stdout, seedname)   ! Ivo: not used in postw90, right?
    call comms_bcast(atoms%num_species, 1, stdout, seedname) ! Ivo: not used in postw90, right?
    ...
    call comms_bcast(param_input%num_exclude_bands, 1, stdout, seedname)
    if (param_input%num_exclude_bands > 0) then
      ...
      call comms_bcast(param_input%exclude_bands(1), param_input%num_exclude_bands, stdout, seedname)
    end if

    call comms_bcast(param_input%gamma_only, 1, stdout, seedname)
    call comms_bcast(dis_data%win_min, 1, stdout, seedname)
    call comms_bcast(dis_data%win_max, 1, stdout, seedname)
    call comms_bcast(dis_data%froz_min, 1, stdout, seedname)
    call comms_bcast(dis_data%froz_max, 1, stdout, seedname)
    call comms_bcast(dis_data%num_iter, 1, stdout, seedname)
    call comms_bcast(dis_data%mix_ratio, 1, stdout, seedname)
    call comms_bcast(dis_data%conv_tol, 1, stdout, seedname)
    call comms_bcast(dis_data%conv_window, 1, stdout, seedname)
    call comms_bcast(dis_data%spheres_first_wann, 1, stdout, seedname)
    call comms_bcast(dis_data%spheres_num, 1, stdout, seedname)
    if (dis_data%spheres_num > 0) then
      if (.not. on_root) then
        allocate (dis_data%spheres(4, dis_data%spheres_num), stat=ierr)
        if (ierr /= 0) &
      ...
      call comms_bcast(dis_data%spheres(1, 1), 4*dis_data%spheres_num, stdout, seedname)
    end if
    call comms_bcast(param_wannierise%num_iter, 1, stdout, seedname)
    call comms_bcast(param_wannierise%num_cg_steps, 1, stdout, seedname)
    call comms_bcast(param_wannierise%conv_tol, 1, stdout, seedname)
    call comms_bcast(param_wannierise%conv_window, 1, stdout, seedname)
    call comms_bcast(param_wannierise%guiding_centres, 1, stdout, seedname)
    call comms_bcast(w90_calcs%wannier_plot, 1, stdout, seedname) !!BGS!!
    call comms_bcast(param_plot%num_wannier_plot, 1, stdout, seedname)
    if (param_plot%num_wannier_plot > 0) then
      ...
      call comms_bcast(param_plot%wannier_plot_list(1), param_plot%num_wannier_plot, stdout, seedname)
    end if
    call comms_bcast(param_plot%wannier_plot_supercell(1), 3, stdout, seedname)
    call comms_bcast(param_plot%wannier_plot_format, len(param_plot%wannier_plot_format), stdout, seedname)
    call comms_bcast(param_plot%wannier_plot_mode, len(param_plot%wannier_plot_mode), stdout, seedname)
    call comms_bcast(param_plot%wannier_plot_spinor_mode, len(param_plot%wannier_plot_spinor_mode), stdout, seedname)
    call comms_bcast(param_plot%write_u_matrices, 1, stdout, seedname)
    call comms_bcast(w90_calcs%bands_plot, 1, stdout, seedname)
    call comms_bcast(param_plot%write_bvec, 1, stdout, seedname)
    call comms_bcast(param_plot%bands_num_points, 1, stdout, seedname)
    call comms_bcast(param_plot%bands_plot_format, len(param_plot%bands_plot_format), stdout, seedname)
    call comms_bcast(param_input%bands_plot_mode, len(param_input%bands_plot_mode), stdout, seedname)
    call comms_bcast(param_plot%num_bands_project, 1, stdout, seedname)
    ...
    if (param_plot%num_bands_project > 0) then
      ...
      call comms_bcast(param_plot%bands_plot_project(1), param_plot%num_bands_project, stdout, seedname)
    end if
    call comms_bcast(param_plot%bands_plot_dim, 1, stdout, seedname)
    call comms_bcast(w90_calcs%write_hr, 1, stdout, seedname)
    call comms_bcast(param_plot%write_rmn, 1, stdout, seedname)
    call comms_bcast(param_plot%write_tb, 1, stdout, seedname)
    call comms_bcast(param_input%hr_cutoff, 1, stdout, seedname)
    call comms_bcast(param_input%dist_cutoff, 1, stdout, seedname)
    call comms_bcast(param_input%dist_cutoff_mode, len(param_input%dist_cutoff_mode), stdout, seedname)
    call comms_bcast(param_input%dist_cutoff_hc, 1, stdout, seedname)
    call comms_bcast(one_dim_axis, len(one_dim_axis), stdout, seedname)
    ...
    call comms_bcast(w90_calcs%fermi_surface_plot, 1, stdout, seedname)
    call comms_bcast(fermi_surface_data%num_points, 1, stdout, seedname)
    call comms_bcast(fermi_surface_data%plot_format, len(fermi_surface_data%plot_format), stdout, seedname)
...
! ----------------------------------------------
    ...
    ! [gp-end]
    call comms_bcast(param_input%use_ws_distance, 1, stdout, seedname)
    call comms_bcast(w90_calcs%disentanglement, 1, stdout, seedname)

    call comms_bcast(w90_calcs%transport, 1, stdout, seedname)
    call comms_bcast(tran%easy_fix, 1, stdout, seedname)
    call comms_bcast(tran%mode, len(tran%mode), stdout, seedname)
    call comms_bcast(tran%win_min, 1, stdout, seedname)
    call comms_bcast(tran%win_max, 1, stdout, seedname)
    call comms_bcast(tran%energy_step, 1, stdout, seedname)
    call comms_bcast(tran%num_bb, 1, stdout, seedname)
    call comms_bcast(tran%num_ll, 1, stdout, seedname)
    call comms_bcast(tran%num_rr, 1, stdout, seedname)
    call comms_bcast(tran%num_cc, 1, stdout, seedname)
    call comms_bcast(tran%num_lc, 1, stdout, seedname)
    call comms_bcast(tran%num_cr, 1, stdout, seedname)
    call comms_bcast(tran%num_bandc, 1, stdout, seedname)
    call comms_bcast(tran%write_ht, 1, stdout, seedname)
    call comms_bcast(tran%read_ht, 1, stdout, seedname)
    call comms_bcast(tran%use_same_lead, 1, stdout, seedname)
    call comms_bcast(tran%num_cell_ll, 1, stdout, seedname)
    call comms_bcast(tran%num_cell_rr, 1, stdout, seedname)
    call comms_bcast(tran%group_threshold, 1, stdout, seedname)
    call comms_bcast(param_hamil%translation_centre_frac(1), 3, stdout, seedname)
    call comms_bcast(kmesh_data%num_shells, 1, stdout, seedname)
    call comms_bcast(kmesh_data%skip_B1_tests, 1, stdout, seedname)
    call comms_bcast(driver%explicit_nnkpts, 1, stdout, seedname)

    call comms_bcast(pp_calc%only_A, 1, stdout, seedname)
    call comms_bcast(w90_calcs%use_bloch_phases, 1, stdout, seedname)
    call comms_bcast(driver%restart, len(driver%restart), stdout, seedname)
    call comms_bcast(param_wannierise%write_r2mn, 1, stdout, seedname)
    call comms_bcast(param_wannierise%num_guide_cycles, 1, stdout, seedname)
    call comms_bcast(param_wannierise%num_no_guide_iter, 1, stdout, seedname)
    call comms_bcast(param_wannierise%fixed_step, 1, stdout, seedname)
    call comms_bcast(param_wannierise%trial_step, 1, stdout, seedname)
    call comms_bcast(param_wannierise%precond, 1, stdout, seedname)
    call comms_bcast(param_wannierise%write_proj, 1, stdout, seedname)
    call comms_bcast(param_input%timing_level, 1, stdout, seedname)
    call comms_bcast(param_input%spinors, 1, stdout, seedname)
    call comms_bcast(param_input%num_elec_per_state, 1, stdout, seedname)
    call comms_bcast(param_wannierise%translate_home_cell, 1, stdout, seedname)
    call comms_bcast(param_input%write_xyz, 1, stdout, seedname)
    call comms_bcast(param_wannierise%write_hr_diag, 1, stdout, seedname)
    call comms_bcast(param_wannierise%conv_noise_amp, 1, stdout, seedname)
    call comms_bcast(param_wannierise%conv_noise_num, 1, stdout, seedname)
    call comms_bcast(param_plot%wannier_plot_radius, 1, stdout, seedname)
    call comms_bcast(param_plot%wannier_plot_scale, 1, stdout, seedname)
    call comms_bcast(kmesh_data%tol, 1, stdout, seedname)
    call comms_bcast(param_input%optimisation, 1, stdout, seedname)
    call comms_bcast(param_wannierise%write_vdw_data, 1, stdout, seedname)
    call comms_bcast(param_input%lenconfac, 1, stdout, seedname)
    call comms_bcast(param_wannierise%lfixstep, 1, stdout, seedname)
    call comms_bcast(lsitesymmetry, 1, stdout, seedname)
    call comms_bcast(dis_data%frozen_states, 1, stdout, seedname)
    call comms_bcast(symmetrize_eps, 1, stdout, seedname)

    !vv: Constrained centres
    call comms_bcast(param_wannierise%slwf_num, 1, stdout, seedname)
    call comms_bcast(param_wannierise%slwf_constrain, 1, stdout, seedname)
    call comms_bcast(param_wannierise%slwf_lambda, 1, stdout, seedname)
    call comms_bcast(param_wannierise%selective_loc, 1, stdout, seedname)
    if (param_wannierise%selective_loc .and. param_wannierise%slwf_constrain) then
      if (.not. on_root) then
        allocate (ccentres_frac(num_wann, 3), stat=ierr)
        if (ierr /= 0) call io_error('Error allocating ccentres_frac in param_get_centre_constraints', stdout, seedname)
      ...
      call comms_bcast(ccentres_frac(1, 1), 3*num_wann, stdout, seedname)
      call comms_bcast(param_wannierise%ccentres_cart(1, 1), 3*num_wann, stdout, seedname)
    end if

    ! vv: automatic projections
    call comms_bcast(kmesh_data%auto_projections, 1, stdout, seedname)

    call comms_bcast(num_proj, 1, stdout, seedname)
    call comms_bcast(lhasproj, 1, stdout, seedname)
    if (lhasproj) then
      if (.not. on_root) then
      ...
      call comms_bcast(kmesh_data%input_proj_site(1, 1), 3*num_proj, stdout, seedname)
      call comms_bcast(param_wannierise%proj_site(1, 1), 3*num_wann, stdout, seedname)
    endif
    ...
    if (.not. driver%explicit_nnkpts) then
      ...
      call comms_bcast(kmesh_info%wbtot, 1, stdout, seedname)
    ...
    call comms_bcast(param_wannierise%omega_total, 1, stdout, seedname)
    call comms_bcast(param_wannierise%omega_tilde, 1, stdout, seedname)
    call comms_bcast(param_input%omega_invariant, 1, stdout, seedname)
