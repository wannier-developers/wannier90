...
subroutine wannier_setup(seed__name, mp_grid_loc, num_kpts_loc, &
  ...
  use w90_param_methods, only: param_write_header, param_lib_set_atoms, param_read, param_write, param_dealloc
...
!  seedname="wannier"
  ...
  call param_read(driver, w90_calcs, pp_calc, param_input, param_plot, param_wannierise, &
                  ...
                  num_wann, eigval, mp_grid, num_proj, select_proj, real_lattice, recip_lattice, &
                  spec_points, pw90_calcs, postw90_oper, pw90_common, pw90_spin, pw90_ham, kpath, &
                  kslice, dos_data, berry, spin_hall, gyrotropic, geninterp, boltz, eig_found, &
                  .true.)
  ...
  call param_write(driver, w90_calcs, param_input, param_plot, param_wannierise, lsitesymmetry, &
                   ...
                   dis_data, fermi_surface_data, fermi, tran, atoms, num_bands, num_wann, mp_grid, &
                   num_proj, select_proj, real_lattice, recip_lattice, spec_points, pw90_calcs)
  ...
  if (allocated(param_wannierise%proj_site)) then
    ...
    proj_l_loc(1:num_proj) = driver%proj%l(1:num_proj)
    proj_m_loc(1:num_proj) = driver%proj%m(1:num_proj)
    proj_z_loc(:, 1:num_proj) = driver%proj%z(:, 1:num_proj)
    proj_x_loc(:, 1:num_proj) = driver%proj%x(:, 1:num_proj)
    proj_radial_loc(1:num_proj) = driver%proj%radial(1:num_proj)
    proj_zona_loc(1:num_proj) = driver%proj%zona(1:num_proj)
    if (allocated(driver%proj%s) .and. present(proj_s_loc) .and. present(proj_s_qaxis_loc)) then
      proj_s_loc(1:num_proj) = driver%proj%s(1:num_proj)
      proj_s_qaxis_loc(:, 1:num_proj) = driver%proj%s_qaxis(:, 1:num_proj)
    end if
  endif
...
subroutine wannier_run(seed__name, mp_grid_loc, num_kpts_loc, real_lattice_loc, recip_lattice_loc, &
  ...
  use w90_param_methods, only: param_lib_set_atoms, param_read, param_write, param_write_chkpt, &
    param_dealloc
