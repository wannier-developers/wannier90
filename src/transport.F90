...
!   end w90_parameters
    ...
    if (param_input%dist_cutoff .gt. dist) then
      write (stdout, '(1x,a,1x,F10.5,1x,a)') 'param_input%dist_cutoff', param_input%dist_cutoff, trim(param_input%length_unit), 'is too large'
...
!~     character(len=11)                                      :: unkg_file
  ...
  subroutine tran_lcr_2c2_sort(signatures, num_G, pl_warning, &
    ...
    use w90_param_types, only: transport_type, parameter_input_type, wannier_data_type, &
      atom_data_type
...
!   end w90_parameters
  ...
  subroutine check_and_sort_similar_centres(signatures, num_G, &
    ...
    use w90_param_types, only: transport_type, parameter_input_type, atom_data_type
...
!   end w90_parameters
  ...
  subroutine tran_write_xyz(tran, atoms, &
    ...
    use w90_param_types, only: transport_type, atom_data_type
...
!   end w90_parameters
  ...
  subroutine tran_lcr_2c2_build_ham(pl_warning, &
    ...
    use w90_param_types, only: transport_type, parameter_input_type, k_point_type, &
      fermi_data_type
