...
! a_matrix
  ...
  subroutine overlap_read(lsitesymmetry, m_matrix_orig_local, m_matrix_local, param_input, &
                          ...
                          select_proj, num_proj, kmesh_info, num_kpts, num_wann, num_bands, sym, stdout, seedname)
    !%%%%%%%%%%%%%%%%%%%%%
...
!       call comms_bcast(m_matrix_orig(1,1,1,1),num_bands*num_bands*nntot*num_kpts)
      call comms_scatterv(m_matrix_orig_local, num_bands*num_bands*kmesh_info%nntot*counts(my_node_id), &
          m_matrix_orig, num_bands*num_bands*kmesh_info%nntot*counts, num_bands*num_bands*kmesh_info%nntot*displs, stdout, seedname)
    else
!       call comms_bcast(m_matrix(1,1,1,1),num_wann*num_wann*nntot*num_kpts)
      call comms_scatterv(m_matrix_local, num_wann*num_wann*kmesh_info%nntot*counts(my_node_id), &
                   m_matrix, num_wann*num_wann*kmesh_info%nntot*counts, num_wann*num_wann*kmesh_info%nntot*displs, stdout, seedname)
    endif
    ...
    if (.not. w90_calcs%use_bloch_phases) then
      ...
      if (w90_calcs%disentanglement) then
        call comms_bcast(a_matrix(1, 1, 1), num_bands*num_wann*num_kpts, stdout, seedname)
      else
    ...
    if ((.not. w90_calcs%disentanglement) .and. (.not. w90_calcs%cp_pp) .and. (.not. w90_calcs%use_bloch_phases)) then
      if (.not. param_input%gamma_only) then
        call overlap_project(m_matrix_local, kmesh_info%nnlist, kmesh_info%nntot, m_matrix, u_matrix, &
                             param_input%timing_level, num_kpts, num_wann, num_bands, lsitesymmetry, sym, stdout, seedname)
      else
        call overlap_project_gamma(kmesh_info%nntot, m_matrix, u_matrix, param_input%timing_level, num_wann, stdout, seedname)
      endif
    endif
!~[aam]
...
!       u_matrix(:,:,nkp)=matmul(cz,cvdag)
    ...
    call comms_gatherv(m_matrix_local, num_wann*num_wann*nntot*counts(my_node_id), &
                       m_matrix, num_wann*num_wann*nntot*counts, num_wann*num_wann*nntot*displs, stdout, seedname)
